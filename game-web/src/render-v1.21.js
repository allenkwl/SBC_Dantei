...
const Render = {
  ...
  init(canvas) {
    ...
    // 四種車型正式改用扁平風圖示（「列車.jpg」切出來的四排，在 vehicle_test.html 比對確認過）：
    // local＝慢車（普通車）、express＝快車（區間車）、tzechiang＝自強號（普悠瑪）、hsr＝高鐵。
    // 舊畫風的圖檔還留在 assets/trains/ 底下，只有 vehicle_test.html（走不帶版本號的 render.js）
    // 會載入來做新舊比較，正式遊戲不再載入，省下四張沒用到的圖。
    const TRAIN_FILE = {
      local: 'train_flat_local', local2: 'train_flat_local',
      express: 'train_flat_express',
      tzechiang: 'train_flat_tzechiang', puyuma: 'train_flat_tzechiang',
      hsr: 'train_flat_hsr', hsr2: 'train_flat_hsr',
    };
