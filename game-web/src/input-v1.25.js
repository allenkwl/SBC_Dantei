...
const Input = {
  ...
  // 放開就清掉紀錄。方向鍵和確定/返回/卡片等全部按鍵共用同一套，repeat 從 155ms
  // 調成 200ms，讓一般單按一下不會太容易誤觸自動連發。
  ...
  poll() {
    ...
    if (pad) {
      ...
      this.pulse('a', b(map.confirm), ' ', 'Space');
      this.pulse('b', b(map.back), 'b', 'KeyB');
      this.pulse('x', b(map.cards), 'x', 'KeyX');
      this.pulse('y', b(map.reach), 'y', 'KeyY');
      this.pulse('lb', b(map.zoom), 'z', 'KeyZ');
      this.pulse('rb', b(map.settings), 'p', 'KeyP');
      this.pulse('select', b(map.mute), 'm', 'KeyM');
    }
    requestAnimationFrame(() => this.poll());
  }
};
