...
const Game = {
  ...
  pushNetState() {
    ...
    Net.uploadState(Object.assign(this.serialize(), {ver, by: Net.clientId}));
  },
  ...
  applyNetState(data) {
    ...
    data.players.forEach((pd, i) => {
      ...
      p.isAI = !!pd.isAI; p.aiLevel = pd.aiLevel || 1;
    });
  ...
  land() {
    ...
    if (pl.pos === this.destination) {
      ...
      this.pushCmd({type: 'celebrate', station: st.id, player: this.cur, bonus});
      UI.showDestinationCelebration(st, pl, bonus, nextId => {
        this.pushCmd({type: 'overlayEnd'});
  ...
  nextPlayer() {
    ...
    if (this.cur === 0) {
      ...
      if (annualData) {
        ...
        this.pushCmd({type: 'annual', data: annualData});
        UI.showAnnualSettlement(annualData, () => { this.pushCmd({type: 'overlayEnd'}); startNewMonth(); });
      } else {
        startNewMonth();
      }
    } else {
  ...
  handOffToken() {
    ...
    const claim = this.netClaims && this.netClaims[this.cur];
    const next = (claim && claim.id) ? claim.id : (Net.hostId || Net.clientId);
    const turn = (Net.tokenTurn || 0) + 1;
    if (next === Net.clientId) {
      // 還是自己（電腦回合而我是群主）：更新序號就好，不用交接，也不用等 ack
      Net.assignToken(next, turn, this.cur);
      return;
    }
    Net.assignToken(next, turn, this.cur).then(ok => {
