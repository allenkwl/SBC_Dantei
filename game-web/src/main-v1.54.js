...
addEventListener('DOMContentLoaded', () => {
  ...
  function openRoom() {
    ...
    Net.watchRoom(info => {
      ...
      // 同一個 watcher 也順便盯著「正式遊戲中有沒有人斷線」，涵蓋整場遊戲，不用另外訂閱。
      checkDisconnect(info);
    });
  ...
  // 遊戲進行中偵測斷線：只在真的開始玩之後才檢查（狀態是 started），大廳／等待室
  // 階段本來就會有人進進出出，不算斷線。這個 watcher 從 openRoom 就一直掛著，涵蓋
  // 選角畫面與正式遊戲全程，不用另外再訂閱一次。
  //
  ...
  // started」，不用等真正進到遊戲畫面。
  function checkDisconnect(info) {
    if (info.status !== 'started') return;
