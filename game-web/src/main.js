...
addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('#setup .n-btn').forEach(btn => {
    btn.onclick = () => {
      ...
      showRoster(parseInt(btn.dataset.n));
    };
  });
  ...
  function renderRoster() {
    ...
    picks.forEach((ci, i) => {
      ...
      const pickerHTML = CHARS.map((c, ci2) => `<img class="roster-avatar-opt${ci2 === ci ? ' selected' : ''}" data-idx="${i}" data-ci="${ci2}" src="${c.avatar}" title="${c.name}" alt="${c.name}">`).join('');
      row.innerHTML = `
  ...
  document.getElementById('btn-settings').onclick = () => {
    ...
    if (pl) {
      ...
      div.innerHTML = `
        ...
        <b>${pl.name}</b>${pl.isAI ? '<span class="ai-tag">CPU</span>' : ''}
    ...
    data.players.forEach(p => {
      ...
      div.innerHTML = `
        <div class="sd-player-head">
          ...
          <b>${p.name}</b>${p.isAI ? '<span class="ai-tag">CPU</span>' : ''}
