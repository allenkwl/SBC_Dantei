...
const UI = {
  ...
  showShortcutFlight(pl, card, done) {
    ...
    BGM.unduck();   // 不綁音效：壓低與解除必須成對，否則音樂會一直停在壓低狀態
    const place = (x, y) => {
    ...
    this._sfTimer = setTimeout(() => {
      ...
      // 讓路刻意**不**綁在音效有沒有載到上面。
      // 原本寫成 `if (this._heliSfx) BGM.duck(null)`，而 SFX.loop() 在音檔還沒解碼好
      // 或 fetch 失敗時回傳 null——結果變成「音效載不到時，連讓路也一起不做」。
      // 讓路是畫面演出的一部分，跟音效檔案載入成功與否沒有關係。
      BGM.duck(null);
