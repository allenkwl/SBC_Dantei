...
const Render = {
  ...
  // ── 依「畫面上真的有什麼在動」決定這一幀要不要畫 ────────────────────
  // 以前是無條件每秒 60 次重畫整張地圖（還外加小地圖），就算畫面完全靜止也照畫，
  // 手機會一直發熱。整份 render 只有兩個東西跟時間有關：目的地旗子的浮動（而且只有
  // 目的地在畫面內時才畫，在畫面外畫的邊緣箭頭是靜態的）、以及可達站點的脈動光圈。
  // 其他全部是靜態的，只有鏡頭或棋子移動時才需要重畫。所以分三段：
  //   30fps 移動中（棋子動畫／鏡頭補間／方向鍵捲動）——板遊不是動作遊戲，30 就夠
  //   12fps 只有旗子或光圈在動
  //    2fps 完全靜止
  // 最低保留 2fps 而不是真的停止，是刻意的：有些畫面變化不經過動畫也不動鏡頭
  // （最典型是連線時別人買下物產，站點顏色要跟著變），留 2fps 底噪就能在 0.5 秒內
  // 自動補上，代價只有約 0.16% CPU。這樣就不需要在每個會改變畫面的地方補「請重畫」
  // 呼叫——那種寫法只要漏掉一處，畫面就會凍住。
  FPS_BOOST: 60, FPS_MOVING: 30, FPS_ANIM: 12, FPS_STILL: 2,
  CAM_EPS: 0.05, CAM_SCALE_EPS: 0.001,
  _drawnCam: null,      // 上一次真的畫出去時的鏡頭值
  _boostUntil: 0,
  // 過場動畫（例如捷徑卡的直升機）自己驅動鏡頭、只有短短幾秒又是視覺重點，
  // 呼叫這個就能在期間內維持滿幀，不被省電邏輯降級。時間到自動失效，不需要關掉。
  boost(ms = 300) {
    const until = performance.now() + ms;
    if (until > this._boostUntil) this._boostUntil = until;
  },
  camMoving() {
    const c = this.cam;
    // ① 補間進行中：目標值跟目前值還有距離
    if (Math.abs(c.tx - c.x) > this.CAM_EPS || Math.abs(c.ty - c.y) > this.CAM_EPS
        || Math.abs(c.tscale - c.scale) > this.CAM_SCALE_EPS) return true;
    // ② 「瞬間貼齊」式的移動：呼叫端把 cam.x 與 cam.tx 同時設成新值（snapTo()、
    //    或直升機捷徑那種每幀把鏡頭釘在自己身上的寫法）。這種情況①永遠看不出來，
    //    差距恆為 0，會被誤判成靜止——所以再跟「上次真的畫出去時的值」比一次。
    //    這樣不管鏡頭是用什麼方式被移動的都抓得到，不必去改每個呼叫端。
    const L = this._drawnCam;
    if (!L) return false;
    return Math.abs(c.x - L.x) > this.CAM_EPS || Math.abs(c.y - L.y) > this.CAM_EPS
        || Math.abs(c.scale - L.scale) > this.CAM_SCALE_EPS;
  },
  // 目的地旗子只有在畫面內才會浮動；在畫面外畫的是靜態箭頭，不需要連續重畫。
  destFlagVisible() {
    if (typeof Game === 'undefined' || !Game.destination) return false;
    const st = Data.stations.get(Game.destination);
    if (!st) return false;
    const s = this.worldToScreen(st.x, st.y);
    const margin = 46 * devicePixelRatio;
    return s.x >= margin && s.x <= this.canvas.width - margin
        && s.y >= margin && s.y <= this.canvas.height - margin;
  },
  targetFps() {
    const k = this.panKeys;
    if (performance.now() < this._boostUntil) return this.FPS_BOOST;
    if (this.anims.length || this.camMoving() || k.left || k.right || k.up || k.down) return this.FPS_MOVING;
    if (this.reachableStations.size || this.scoutStation || this.destFlagVisible()) return this.FPS_ANIM;
    return this.FPS_STILL;
  },

  frame() {
    // 迴圈本身永遠照跑，只是跳過「繪製」——狀態一變下一幀就自動偵測到，
    // 不需要任何喚醒機制。
    requestAnimationFrame(() => this.frame());
    // 刻意不檢查 document.hidden：分頁被切到背景時瀏覽器本來就會自己把 rAF 節流到
    // 幾乎不跑，省電效果已經有了；反而是某些 WebView／自動化環境會在前景時也回報
    // hidden=true，真的照它跳過繪製就會整個畫面凍住，風險大於好處。
    const nowT = performance.now();
    if (this.lastDrawT && nowT - this.lastDrawT < 1000 / this.targetFps() - 1) return;
    this.lastDrawT = nowT;
    ...
    // 鏡頭補間。原本是「每一幀收斂 14%」，那是綁在 60fps 上的寫法——幀率一降到 30，
    // 同樣的位移就要花兩倍時間，鏡頭平移縮放會整個變鈍。改成換算成「每 16.7ms 收斂
    // 14%」的等效值，不管現在跑幾 fps，走完同一段距離的實際秒數都一樣。
    const ease = (per60, ms) => 1 - Math.pow(1 - per60, ms / 16.667);
    const kPos = ease(0.14, dt), kScale = ease(0.16, dt);
    cam.x += (cam.tx - cam.x) * kPos;
    cam.y += (cam.ty - cam.y) * kPos;
    cam.scale += (cam.tscale - cam.scale) * kScale;
    // 指數逼近永遠不會真的相等，差距夠小就直接吸附到目標值——否則 camMoving() 永遠
    // 成立，畫面就永遠降不到最低頻率。
    if (Math.abs(cam.tx - cam.x) <= this.CAM_EPS) cam.x = cam.tx;
    if (Math.abs(cam.ty - cam.y) <= this.CAM_EPS) cam.y = cam.ty;
    if (Math.abs(cam.tscale - cam.scale) <= this.CAM_SCALE_EPS) cam.scale = cam.tscale;
    // 記下「這一幀實際畫出去的鏡頭位置」，下一幀 camMoving() 拿它來比對，
    // 才抓得到那種目前值與目標值被同時改掉的瞬間移動（見 camMoving 的說明）。
    this._drawnCam = {x: cam.x, y: cam.y, scale: cam.scale};
    ...
    // 小地圖跟著一起重畫。它也只在棋子或鏡頭有變動時才需要更新，所以被上面的
    // 跳幀一併省下來（排下一幀的 requestAnimationFrame 已經移到 frame() 開頭，
    // 確保就算這一幀跳過繪製、迴圈也不會斷掉）。
