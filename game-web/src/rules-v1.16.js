...
const Game = {
  ...
  maybeAutoRoll() {
    ...
    setTimeout(() => {
      ...
      if (profile.useCards && !pl.cardUsedThisTurn) {
        const cardIdx = pl.cards.findIndex(e => ['highspeed','puyuma','QuickVehicle','hsrPass','puyumaPass','QuickVehiclePass','nightMarket','lucky'].includes(this.cardIdOf(e)));
  ...
  useCard(index, option = null) {
    ...
    switch (card.effect) {
      ...
      case 'teleport': this.teleportPlayer(pl, card.target); return;
  ...
  roll(fromCard = false) {
    ...
    pl.forcedDice = null; pl.doubleDice = false; pl.tripleDice = false; pl.fiveDice = false; pl.moveBonus = 0; pl.nextDicePenalty = 0;
