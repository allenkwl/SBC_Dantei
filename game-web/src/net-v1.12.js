...
const Net = {
  ...
  disbandGroup(key) {
    ...
    return Promise.all([
      this.db.ref('groups/' + k).remove().catch(() => {}),
      this.db.ref('ready/' + k).remove().catch(() => {}),
      this.db.ref('claims/' + k).remove().catch(() => {}),
      this.db.ref('pause/' + k).remove().catch(() => {}),
      this.db.ref('live/' + k).remove().catch(() => {}),
    ]);
  },
