...
const Game = {
  ...
  applyTurnPresentation(prev) {
    ...
    if (curChanged) {
      // 汽笛「噗噗」＝該出發了，要在**列車真的出現在畫面上**的那一刻響。
      // 跨月時中間隔著月曆橫幅（跨年還會先夾一段年度結算，要等按 A、時間不固定），
      // 所以有橫幅的情況交給 UI.showMonthBanner 在動畫結束時發聲，這裡只處理沒有橫幅的一般換人。
      if (!monthChanged) SFX.trainWhistle();
      UI.clearDice();
      if (!Render.freeLook) {
  ...
  updateVehicleBGM(mode) {
    ...
    if (key !== prev) {
      if (key === 'sea') SFX.shipHorn();
      else if (key === 'plane') SFX.planeTakeoff();
    }
  },
