...
const UI = {
  ...
  // 結果會停留到玩家按任意鍵；按鍵後淡出，並把同一個鍵轉交給移動流程。
  showDice(finalValues, total, done) {
    const el = document.getElementById('dice');
    const render = (values, waiting = false) => { el.innerHTML = `<div class="dice-cup dice-${values.length}">${values.map(n => `<span class="die face-${n}"></span>`).join('')}</div><small>合計 ${total} 點${waiting ? '　按任意鍵繼續' : ''}</small>`; };
    ...
    this._diceDone = setTimeout(() => {
      ...
      showFaces(finalValues); el.classList.remove('rolling'); el.classList.add('settled', 'await-key');
      el.querySelector('small').textContent = `合計 ${total} 點　按任意鍵繼續`;
      this._diceAwaiting = {el, done};
    }, 800);
  },
  ...
  dismissDiceAfterKey(event) {
    ...
    setTimeout(() => {
      ...
      // 讓觸發淡出的同一顆鍵在岔路按鈕產生後重新執行；合成事件只走既有鍵盤邏輯。
