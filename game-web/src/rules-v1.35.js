...
const Game = {
  ...
  isNetDriver() {
    ...
    if (Seats.byPlayer && Seats.byPlayer.has(this.cur)) return true;
    return typeof Net !== 'undefined' && Net.isHost;  // 電腦角色交給群主跑
  },
  ...
  nextPlayer() {
    ...
    this.pushNetState();
    // 標記「這一局還有人在玩」。幽靈群組的判定就是靠它：成員節點還在、但這個時間戳
    // 很久沒更新，就代表那些人其實早就離開了（見 net.js 的 openOrReclaim）。
    // 用回合換人當觸發點而不是計時器：正常對局每幾十秒就換一次人，不用另外開計時器
    // 定期寫入，也就不會回到「閒置時仍持續寫」的老問題。
    if (this.netGroup && this.isNetDriver() && typeof Net !== 'undefined') Net.markActive();
