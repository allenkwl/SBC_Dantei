...
addEventListener('DOMContentLoaded', () => {
  ...
  // 哪些畫面不該顯示它：
  //  ‧ 主畫面：已經是最外層，而且那裡「按任意鍵」會直接進下一步，B 不是返回
  //  ‧ 序章：B 是「跳過序章」，掛個返回鈕語意不對
  //  ‧ 改名面板：它自己有明確的「✕ 取消」，而且鍵盤打字時 b 是字元不是返回
  // 其餘畫面（設定、選角、存檔匣、大廳、房間、遊戲中）一律顯示。
  // 用輪詢而不是在每個切換畫面的地方各加一行：畫面切換的呼叫點散在很多檔案裡，
  // 漏掉一處就會出現「該顯示卻沒顯示」的死角，200ms 檢查一次成本可以忽略且不會漏。
  const NO_BACK = ['splash', 'intro', 'vkbd'];
  setInterval(() => {
    const blocked = NO_BACK.some(id => {
      const el = document.getElementById(id);
      return el && getComputedStyle(el).display !== 'none';
    });
    backBtn.classList.toggle('show', !blocked);
  }, 200);
