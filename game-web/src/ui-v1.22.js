...
const UI = {
  ...
  init() {
    ...
    document.addEventListener('keydown', e => {
      ...
      // 不能靠瀏覽器原生的 showPicker() 彈出視窗，那個彈出視窗只認真的鍵盤/滑鼠輸入，
      // 手把在 input.js 是合成事件，開出視窗後手把方向鍵完全無法在裡面選（這是瀏覽器
      // 原生彈出選單的限制，不是我們的鍵盤監聽器能接住的範圍），直接改值才能鍵盤／手把都通用。
      // 確定鍵維持另外叫出 showPicker()，給用滑鼠/鍵盤想直接看完整清單的人用，非必要操作。
      // 左右在三個下拉選單／傳送鈕之間移動；焦點在卡片清單（格狀排列）時方向鍵沿用跟卡片商店
      // 同一套「抓畫面上最近的那個」邏輯。
