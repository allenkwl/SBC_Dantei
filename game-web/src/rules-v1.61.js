...
const Game = {
  ...
  updateVehicleBGM(mode) {
    ...
    const prev = this._bgmKey;
    this._bgmKey = key;
    BGM.play(key);
    // 剛上船／剛起飛的那一刻各響一次。用「跟上一次的曲子不同」當邊緣觸發——
    // 這個函式在移動中每一步都會被呼叫，不判斷變化的話會一路連續鳴笛。
    // driver 走 stepTo、listener 走 syncBgm，兩邊都經過這裡，所以兩邊都會聽到。
    if (key !== prev) this.vehicleCue(mode);
  },
