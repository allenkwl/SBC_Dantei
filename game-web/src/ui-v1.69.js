...
// 到站演出的列車圖：依 pl.train（本回合實際使用的車型）切換，皆為劇場版雙節去背圖、不套用玩家顏色。
// tw/tto/tb/tdur/texit/toutdur 對應 #dc-train 的 CSS 變數（寬度／停靠 right／底部／進站秒數／
// 離場 right／離場秒數）。正常情況下實際數值一律來自 arrival_settings.js 的 trains 欄位（見
// trainArrivalProfile()）——這裡只是那份共用設定讀不到時的備援值，跟模擬器 v1.9 的預設一致。
const TRAIN_ARRIVAL_PROFILES = {
  local:     { src:'../圖片/慶祝到站列車/劇場版雙節v0.1/慢車-雙節.png',   tw:150, tto:-55, tb:-10, tdur:6.0, texit:115, toutdur:4.8 },
  express:   { src:'../圖片/慶祝到站列車/劇場版雙節v0.1/快車-雙節.png',   tw:180, tto:-85, tb:-35, tdur:4.7, texit:115, toutdur:3.8 },
  tzechiang: { src:'../圖片/慶祝到站列車/劇場版雙節v0.1/普悠瑪-雙節.png', tw:180, tto:-85, tb:-40, tdur:3.8, texit:115, toutdur:3.2 },
  hsr:       { src:'../圖片/慶祝到站列車/劇場版雙節v0.1/高鐵-雙節.png',   tw:170, tto:-80, tb:-10, tdur:2.9, texit:115, toutdur:2.7 },
};
// 讀 arrival_settings.js 的 trains[trainId]（模擬器存的欄位：image/width/stopRight/bottom/
// arrivalDuration/exitRight/departureDuration），數值格式不對或整段缺漏就退回上面的備援值。
function trainArrivalProfile(trainId) {
  const fallback = TRAIN_ARRIVAL_PROFILES[trainId] || TRAIN_ARRIVAL_PROFILES.local;
  const raw = window.ARRIVAL_SETTINGS && window.ARRIVAL_SETTINGS.trains && window.ARRIVAL_SETTINGS.trains[trainId];
  if (!raw || typeof raw.image !== 'string') return fallback;
  const merged = {
    src: raw.image, tw: Number(raw.width), tto: Number(raw.stopRight), tb: Number(raw.bottom),
    tdur: Number(raw.arrivalDuration), texit: Number(raw.exitRight), toutdur: Number(raw.departureDuration),
  };
  const nums = [merged.tw, merged.tto, merged.tb, merged.tdur, merged.texit, merged.toutdur];
  return nums.every(Number.isFinite) ? merged : fallback;
}
const DEST_CHEER_STOP_DELAY = 3650;   // 跟「按 A 揭曉下一站」提示同時消音，同模擬器 PROMPT_DELAY
// 到站演出的節奏（提示延遲／繼續提示最短延遲／離場後緩衝），同樣改讀 arrival_settings.js
// 的 flow 欄位，讀不到才用這組預設（跟模擬器 v1.9 的 FLOW_DEFAULT 一致）。
const ARRIVAL_FLOW_DEFAULT = {promptDelay: 3650, nextRevealDelay: 500, departurePromptBuffer: 250};
function arrivalFlowValue(key) {
  const shared = window.ARRIVAL_SETTINGS && window.ARRIVAL_SETTINGS.flow;
  const v = shared && Number(shared[key]);
  return Number.isFinite(v) ? v : ARRIVAL_FLOW_DEFAULT[key];
}
...
const UI = {
  ...
  // 到站演出的小朋友、「抵達目的地」文字與車站時鐘疊圖位置，改讀 arrival_settings.js
  // （跟「目的地慶祝動畫模擬」共用同一份檔案，存檔即生效）。找不到共用設定（例如用
  // file:// 直接雙擊打開）才退回下面這組內建預設值，行為不會壞掉。
  ARRIVAL_SHARED_KEY: 'xiaoqiu-cat-railway-arrival-shared-v1',
  ARRIVAL_KID_IMAGES: [
    '../圖片/慶祝到站小朋友/劇場版v0.1/小朋友01-黃外套男孩.png', '../圖片/慶祝到站小朋友/劇場版v0.1/小朋友02-紅外套女孩.png',
    '../圖片/慶祝到站小朋友/劇場版v0.1/小朋友03-藍帽男孩.png', '../圖片/慶祝到站小朋友/劇場版v0.1/小朋友04-黃外套女孩.png',
    '../圖片/慶祝到站小朋友/劇場版v0.1/小朋友05-紅帽男孩.png', '../圖片/慶祝到站小朋友/劇場版v0.1/小朋友06-草帽女孩.png',
    '../圖片/慶祝到站小朋友/劇場版v0.1/小朋友07-眼鏡男孩.png', '../圖片/慶祝到站小朋友/劇場版v0.1/小朋友08-和服女孩.png',
  ],
  ...
  ARRIVAL_DEFAULT_KIDS: [
    {img: 0, left: 0,  bottom: 13, width: 14}, {img: 1, left: 8,  bottom: 13, width: 14},
    {img: 2, left: 16, bottom: 13, width: 14}, {img: 3, left: 24, bottom: 13, width: 14},
    {img: 4, left: 2,  bottom: -5, width: 17}, {img: 5, left: 10, bottom: -5, width: 17},
    {img: 6, left: 18, bottom: -5, width: 17}, {img: 7, left: 26, bottom: -5, width: 17},
  ],
  ARRIVAL_DEFAULT_TITLE: {top: 25, left: 73, scale: 1},
  ARRIVAL_DEFAULT_CLOCK: {left: 49.9, top: 15.9, size: 17},
  initArrivalShared() {
    ...
    const clockShared = shared && shared.clock;
    const clock = (clockShared && ['left', 'top', 'size'].every(k => Number.isFinite(Number(clockShared[k]))))
      ? clockShared : this.ARRIVAL_DEFAULT_CLOCK;
    box.innerHTML = '';
    kids.forEach((k, i) => {
    ...
    const clockEl = document.getElementById('dc-clock');
    if (clockEl) {
      clockEl.style.setProperty('--clock-left', clock.left + '%');
      clockEl.style.setProperty('--clock-top', clock.top + '%');
      clockEl.style.setProperty('--clock-width', clock.size + '%');
      clockEl.style.setProperty('--clock-height', (clock.size * 1.5) + '%');
    }
  },
    ...
    if (pl) {
      ...
      div.innerHTML = `${avatarImg}
        <div class="p-cur-info">
          <div class="p-cur-name"><b>${pl.name}</b>${
              // 貓咪本名只在 HUD 出現。理由是這裡頭像就在旁邊，名字跟頭像配成一對，
              // 玩家才會記住「原來這隻是賓士」；折線圖與排名要回答的是「誰贏」，
              // 放本名只是雜訊。名字沒改過時 charNameOf 回空字串，不會變成「探險家（探險家）」。
              (() => { const cn = Game.charNameOf(pl); return cn ? `<span class="p-cur-char">（${cn}）</span>` : ''; })()
            }${pl.isAI ? `<span class="ai-tag">${(AI_PROFILES[pl.aiLevel] || AI_PROFILES[1]).label}</span>` : ''}</div>
  ...
  // ms：停留多久（含淡入淡出），預設 1550 跟 CSS 的 --cf-dur 預設值一致。
  //     捷徑卡的演出依賴「提示剛好 1550ms 後消失」才接著讓直升機起飛，所以預設值不能改。
  // opts.localOnly：不要再廣播（收到別人廣播而顯示時用，免得又轉發一次）
  CARD_FLASH_MS: 1550,
  showCardFlash(icon, text, ms, opts) {
    const dur = ms || this.CARD_FLASH_MS;
    if (!(opts && opts.localOnly)) this._broadcast({type: 'cardFlash', args: [icon, text, dur]});
    ...
    clearTimeout(this._cardFlashTimer); this._cardFlashTimer = setTimeout(() => el.classList.remove('playing'), dur);
  },
  ...
  applyDestReveal(d) {
    ...
    if (d.nx && this._destNextId !== d.nx) {
      ...
      if (next) {
        ...
        // driver 揭曉下一站的同時列車也開始離場，listener 這裡要跟著加同一個 class
        // 才會看到一樣的動畫（列車圖與 --texit/--toutdur 已經在 showDestinationCelebration
        // 這邊的 unpack 呼叫時設好，這裡只需要補觸發 .depart）。
        const train = document.getElementById('dc-train');
        if (train) train.classList.add('depart');
      }
    }
  ...
  // 有人回來的宣布。兩種情況要講不一樣的話，因為意思真的不同：
  //   takeover=true  電腦已經代打過幾回合，現在把角色還給他
  //   takeover=false 全場只是暫停等他，沒有人代打過
  // 用畫面中央的大字（跟颱風假、防禦卡同一個介面）停 4 秒，不用 toast——
  // toast 只有 2.6 秒又在畫面邊緣，手機上一晃就過去了（使用者回報「似乎沒有跳出」）。
  REJOIN_MS: 4000,
  announceRejoin(names, opts) {
    const who = names.join('、');
    const takeover = !!(opts && opts.takeover);
    const msg = takeover
      ? `${who} 回來接手了，電腦不再代打`
      : `${who} 回來了，繼續遊戲`;
    // 廣播（路 B）不能走 _broadcast——那個只有「持有 token」的裝置會發，而路 B 的
    // 呼叫者是群主，群主當下不一定握著 token（token 在輪到的那位手上）。
    // 直接送一次指令，指令會回送給自己，所以這裡不重複顯示：全場含群主與回來的那位
    // 都由同一則指令驅動，內容與時間點完全一致。
    if (opts && opts.broadcast) {
      if (typeof Net !== 'undefined' && Net.pushCmd) {
        Net.pushCmd({type: 'cardFlash', args: ['🎮', msg, this.REJOIN_MS]});
        return;
      }
    }
    // 路 A：每台各自偵測到就自己顯示，不必也不能廣播——廣播會讓已經自己顯示過的
    // 裝置再跳第二次。
    this.showCardFlash('🎮', msg, this.REJOIN_MS, {localOnly: true});
    ...
    if (note && banner && banner.classList.contains('playing')) {
      ...
      this._bannerNoteTimer = setTimeout(() => note.classList.remove('show'), this.REJOIN_MS);
    }
  },
  ...
  closeNetOverlays() {
    ...
    this.NET_PANELS.map(d => d.el).forEach(id => {
      // 直升機不是「面板」，它是靠 .show class 控制 opacity 的動畫演出，CSS 裡沒有
      // 任何 display 規則。被寫進 style.display='none' 之後**沒有任何地方會清掉**
      // （showShortcutFlight 只動 CSS 變數／classList／座標），於是這台裝置的直升機從此
      // 永久消失——飛行邏輯照跑，所以症狀是「畫面自己捲過去，但看不到直升機」。
      // 它本來就會在飛行結束時自己 classList.remove('show') 收尾，不需要外力關。
      if (id === 'sf-heli') return;
      const el = document.getElementById(id);
  ...
  showDestinationCelebration(st, pl, bonus, done) {
    ...
    const profile = trainArrivalProfile(pl.train);
    this._destTrainProfile = profile;   // revealNextDestination 算離場時間要用同一份
    ...
    train.style.setProperty('--texit', profile.texit + '%');
    train.style.setProperty('--toutdur', profile.toutdur + 's');
    ...
    train.classList.remove('arrive', 'depart');
  ...
  // v1.6 同一套（起飛前先原地加速旋翼「spool」，再爬升「lift」，巡航時沿垂直方向弓出一個弧線
  // 並在起訖各帶一點機身傾角，抵達後短暫「hover」，最後旋翼減速「land」）——比舊版單張圖片
  // 切換＋純縮放豐富，鏡頭在巡航階段跟著直升機即時移動，起降階段鏡頭停在原地不跟。
  // 這裡不重畫假地圖，直接疊在正式棋盤 canvas 上，座標用 Render.worldToCss 換算；
  // 「使用卡片」的提示沿用既有的 UI.showCardFlash，不用另外做一套卡片看板。
  SF_TIMING: {spool: 520, lift: 430, hover: 170, land: 460},
  showShortcutFlight(pl, card, done) {
    ...
    const craft = heli.querySelector('.sf-craft');
    const shadow = document.getElementById('sf-ground-shadow');
    // 保險：這個元素的可見性走 .show／opacity，一旦有人（例如 closeNetOverlays）
    // 在它身上寫了 inline display:none，就再也沒有人會清掉。每次起飛前歸零一次，
    // 以後再有新的收尾邏輯掃到它也不會壞。
    heli.style.display = '';
    ...
    const place = (x, y) => {
      ...
      heli.style.left = s.x + 'px'; heli.style.top = s.y + 'px';
      if (shadow) { shadow.style.left = s.x + 'px'; shadow.style.top = (s.y + 38) + 'px'; }
    };
    const clamp01 = n => Math.max(0, Math.min(1, n));
    const setAltitude = v => {
      const a = clamp01(v);
      heli.style.setProperty('--altitude', a.toFixed(3));
      heli.style.setProperty('--flight-scale', (1 + a * .13).toFixed(3));
      if (shadow) {
        shadow.style.setProperty('--shadow-scale', (.92 - a * .28).toFixed(3));
        shadow.style.setProperty('--shadow-opacity', (.34 - a * .18).toFixed(3));
      }
    };
    const setRotorBlend = (blades, blur) => {
      heli.style.setProperty('--blade-opacity', clamp01(blades).toFixed(3));
      heli.style.setProperty('--blur-opacity', clamp01(blur).toFixed(3));
    };
    const setFlightVisual = mode => {
      heli.classList.toggle('sf-rotors-on', mode !== 'off');
      heli.classList.toggle('sf-airborne', mode === 'flight');
      heli.style.setProperty('--rotor-speed', mode === 'flight' ? '.105s' : mode === 'landing' ? '.18s' : '.28s');
    };
    if (craft) craft.style.setProperty('--flight-dir', dest.x > origin.x ? -1 : 1);
    heli.style.setProperty('--pitch', '0deg');
    heli.classList.remove('sf-rotors-on', 'sf-airborne');
    heli.classList.add('show');
    if (shadow) shadow.classList.add('show');
    place(origin.x, origin.y);
    setAltitude(0); setRotorBlend(1, 0); setFlightVisual('off');
    ...
    this._sfTimer = setTimeout(() => {
      ...
      const timing = this.SF_TIMING;
      const dist = Math.hypot(dest.x - origin.x, dest.y - origin.y);
      const flightDuration = Math.max(2600, dist / 95 * 1000);
      const dx = dest.x - origin.x, dy = dest.y - origin.y, distTotal = Math.max(1, Math.hypot(dx, dy));
      const ux = dx / distTotal, uy = dy / distTotal, arc = Math.min(62, distTotal * .105);
      const smooth = n => { n = clamp01(n); return n * n * (3 - 2 * n); };
      const easeInOut = n => n < .5 ? 4 * n * n * n : 1 - Math.pow(-2 * n + 2, 3) / 2;
      const t0 = performance.now();
      const flightStart = timing.spool + timing.lift;
      const flightEnd = flightStart + flightDuration;
      const landStart = flightEnd + timing.hover;
      const total = landStart + timing.land;
      const tick = () => {
        // 這段演出是自己驅動鏡頭的（下面每幀把 Render.cam 釘在直升機身上），而且鏡頭
        // 鎖住之後真正在動的是底下整張地圖。render 的省電降幀必須在這幾秒維持滿幀，
        // 否則地圖會變成每秒兩張的幻燈片。每幀續期一次，飛完就自動失效，不用關。
        Render.boost(200);
        const elapsed = performance.now() - t0;
        if (elapsed < timing.spool) {
          const p = smooth(elapsed / timing.spool);
          setAltitude(0); setRotorBlend(1 - p, p);
        } else if (elapsed < flightStart) {
          const p = smooth((elapsed - timing.spool) / timing.lift);
          setAltitude(p); setRotorBlend(0, 1); setFlightVisual('flight');
        } else if (elapsed < flightEnd) {
          const raw = (elapsed - flightStart) / flightDuration, p = easeInOut(raw), bow = Math.sin(Math.PI * p) * arc;
          const x = origin.x + dx * p - uy * bow, y = origin.y + dy * p + ux * bow;
          pl.ax = x; pl.ay = y; place(x, y);
          setAltitude(1); setRotorBlend(0, 1);
          const pitch = raw < .18 ? -5 * (1 - raw / .18) : raw > .78 ? 4 * ((raw - .78) / .22) : 0;
          heli.style.setProperty('--pitch', pitch.toFixed(2) + 'deg');
          Render.cam.x = Render.cam.tx = x; Render.cam.y = Render.cam.ty = y;
        } else if (elapsed < landStart) {
          pl.ax = dest.x; pl.ay = dest.y; place(dest.x, dest.y);
          setAltitude(1); setRotorBlend(0, 1); heli.style.setProperty('--pitch', '0deg');
        } else if (elapsed < total) {
          const p = smooth((elapsed - landStart) / timing.land);
          place(dest.x, dest.y);
          setAltitude(1 - p); setRotorBlend(p, 1 - p); setFlightVisual('landing');
        } else {
          setAltitude(0); setRotorBlend(1, 0); heli.style.setProperty('--pitch', '0deg'); setFlightVisual('off');
          heli.classList.remove('show'); if (shadow) shadow.classList.remove('show');
        ...
        this._sfFrame = requestAnimationFrame(tick);
      };
      tick();
    }, 1550);
  },
  ...
  revealNextDestination(src) {
    ...
    // 列車在揭曉下一站的同時加速離場（見 #dc-train.depart），要等它真的離開畫面
    // 才顯示「繼續旅程」提示，不然車還停在原地、字卻先跳出來會很怪。
    const train = document.getElementById('dc-train');
    if (train) train.classList.add('depart');
    const departMs = ((this._destTrainProfile && Number(this._destTrainProfile.toutdur)) || 3.4) * 1000;
    const continueDelay = Math.max(arrivalFlowValue('nextRevealDelay'), departMs + arrivalFlowValue('departurePromptBuffer'));
    const pl = Game.curPlayer();
  ...
  // ────────────────────────────────────────────────
  //  總決算慶祝畫面（搬自「總決算動畫模擬」v1.2，邏輯原封不動，只換了三件事：
  //   ① 金額格式改用遊戲既有的 formatMoney()，跟 HUD／排名同一套，不再自己算億／萬
  //      ——sim 版的 money() 假設輸入是「元」，但這個專案內部金額全部是「萬」為單位
  //      （MONEY.start=100 對照 HUD「💰100萬」就看得出來），單位對不起來的話數字會全錯。
  //   ② 音效／BGM 改走遊戲既有的 SFX／BGM 物件，不再開獨立的 <audio>——SFX.play() 本來
  //      就會自動 duck 目前的 BGM，不需要另外寫一套 duckBgm()。
  //   ③ 拿掉「開始總決算」的開場票根與逐一切換車型的部分：這裡沒有多種車型可選，
  //      進來就是要看結果，直接播；鍵盤／手把操作交給既有的 overlayControls() 那一套
  //      通用機制（menuOpen 清單裡已經有 'gameover'，方向鍵移動、確定鍵啟用都不用另外寫）。
  // ────────────────────────────────────────────────
  FINALE_ACCENTS: ['#ffd148','#a9cbd8','#e39a58','#7ed9bd','#ff8bac','#a99afa','#73d87d','#bc8b63'],
  _finaleTimers: [], _finaleRAF: 0, _finaleRunning: false, _finaleFinished: false, _finaleRanking: [],

  _finaleLater(fn, ms) { const id = setTimeout(fn, ms); this._finaleTimers.push(id); return id; },

  _finaleBuild() {
    const ranking = this._finaleRanking;
    document.getElementById('year-chip').textContent = `第 ${Game.totalYears} 年・旅程完結`;
    const podium = document.getElementById('podium'), lower = document.getElementById('lower-ranks');
    podium.innerHTML = ''; lower.innerHTML = '';
    const top = ranking.slice(0, 3), visual = [top[1], top[0], top[2]].filter(Boolean);
    visual.forEach((p, vi) => {
      const rank = ranking.indexOf(p) + 1, el = document.createElement('article');
      el.className = `player rank-${rank} ${rank === 1 ? 'first' : ''}`;
      el.dataset.rank = rank;
      el.style.cssText = `--order:${vi};--accent:${this.FINALE_ACCENTS[rank - 1]};--h:${rank === 1 ? '390px' : rank === 2 ? '330px' : '285px'}`;
      el.innerHTML = `<div class="avatar-wrap"><span class="crown">👑</span><img src="${p.avatar}" alt="${p.name}"></div><div class="nameplate">${p.name}<span class="amount">${formatMoney(p.total)}元</span></div><div class="block"><span class="rank-word">FINAL RANK</span>${rank}</div>`;
      podium.appendChild(el);
    });
    ranking.slice(3).forEach((p, i) => {
      const rank = i + 4, el = document.createElement('article');
      el.className = 'mini-player'; el.dataset.rank = rank;
      el.innerHTML = `<span class="mini-rank">${rank}</span><img src="${p.avatar}" alt=""><span class="mini-name">${p.name}<small class="mini-total">${formatMoney(p.total)}元</small></span>`;
      lower.appendChild(el);
    });
    const w = ranking[0];
    document.getElementById('winner-name').textContent = w.name;
    // 特別稱號：這個畫面本來就沒有專屬於「總決算」的稱號系統（TYCOON_TITLES 是三月結算門檻，
    // 跟這裡的第一名不是同一件事），與其硬套一個牽強的規則，不如照 sim 原本的退路——
    // 從幾句通用的祝賀語裡挑一句，不裝作那是遊戲算出來的東西。
    const award = ['環島達人', '投資高手', '幸運旅人', '鐵道之星'][ranking.indexOf(w) % 4];
    document.getElementById('details').innerHTML =
      `<div class="detail"><span class="detail-label">💵 最終現金</span><span class="detail-value">${formatMoney(w.money)}元</span></div>` +
      `<div class="detail"><span class="detail-label">🏪 物產價值・${w.stalls || '—'} 處</span><span class="detail-value">${formatMoney(w.stallsValue)}元</span></div>` +
      `<div class="detail award"><span class="detail-label">本屆特別稱號</span><span class="detail-value">✨ ${award} ✨</span></div>`;
  },

  _finaleParty() {
    const conf = document.getElementById('confetti'), fire = document.getElementById('fireworks');
    conf.innerHTML = ''; fire.innerHTML = '';
    const cs = ['#ffd148', '#ff638c', '#63d7b0', '#7cbbff', '#fff', '#b78cff'];
    for (let i = 0; i < 80; i++) conf.insertAdjacentHTML('beforeend', `<i style="--x:${Math.random()*100}%;--c:${cs[i%cs.length]};--d:${Math.random()*1.8}s;--s:${2.7+Math.random()*2}s;--dx:${-100+Math.random()*200}px"></i>`);
    for (let i = 0; i < 7; i++) fire.insertAdjacentHTML('beforeend', `<i class="burst" style="--x:${12+Math.random()*76}%;--y:${14+Math.random()*42}%;--c:${cs[i%cs.length]};--d:${Math.random()*1.5}s"></i>`);
  },

  _finaleCountTo(total) {
    cancelAnimationFrame(this._finaleRAF);
    const begin = performance.now(), dur = 1750, numEl = document.getElementById('tally-number');
    const tick = now => {
      const t = Math.min(1, (now - begin) / dur), ease = 1 - Math.pow(1 - t, 3);
      numEl.textContent = formatMoney(total * ease) + '元';
      if (t < 1) this._finaleRAF = requestAnimationFrame(tick);
    };
    this._finaleRAF = requestAnimationFrame(tick);
  },

  _finaleReveal(rank) {
    const el = document.querySelector(`#gameover [data-rank="${rank}"]`);
    if (el) el.classList.add('reveal');
  },

  _finaleFinish() {
    if (this._finaleFinished) return;
    this._finaleFinished = true;
    const ranking = this._finaleRanking;
    document.getElementById('tally').classList.remove('show');
    ranking.forEach((_, i) => this._finaleReveal(i + 1));
    document.getElementById('winner-banner').classList.add('show');
    document.getElementById('details').classList.add('show');
    const actions = document.getElementById('actions');
    actions.classList.add('show');
    actions.querySelectorAll('button').forEach(b => { b.disabled = false; });   // 見下方 overlayControls 的說明
    document.getElementById('stage').classList.add('celebrate');
    SFX.play('cheer');   // 內部已經會自動 duck 目前的 BGM，不用另外寫讓路邏輯
    document.getElementById('replay').focus();
  },

  _finaleRun() {
    this._finaleTimers.forEach(clearTimeout); this._finaleTimers = [];
    cancelAnimationFrame(this._finaleRAF);
    this._finaleRunning = true; this._finaleFinished = false;
    this._finaleBuild(); this._finaleParty();
    BGM.play('finale');
    const stage = document.getElementById('stage'), tally = document.getElementById('tally');
    stage.classList.remove('celebrate'); stage.classList.add('running');
    ...
    document.querySelectorAll('#gameover .reveal').forEach(el => el.classList.remove('reveal'));
    const ranking = this._finaleRanking;
    const tallyNumber = document.getElementById('tally-number'), tallyNote = document.getElementById('tally-note');
    this._finaleLater(() => { tally.classList.add('show'); this._finaleCountTo(ranking.reduce((s, p) => s + p.total, 0)); }, 650);
    this._finaleLater(() => { tallyNote.textContent = '排名確認完成，列車即將進站！'; }, 2100);
    const lowerRanks = []; for (let r = ranking.length; r >= 4; r--) lowerRanks.push(r);
    lowerRanks.forEach((r, i) => this._finaleLater(() => this._finaleReveal(r), 2800 + i * 650));
    const base = 2800 + lowerRanks.length * 650;
    this._finaleLater(() => this._finaleReveal(3), base + 300);
    this._finaleLater(() => this._finaleReveal(2), base + 1450);
    this._finaleLater(() => { tally.classList.add('show'); tallyNote.textContent = '最後一位——本屆總冠軍究竟是誰？'; tallyNumber.textContent = '？？？'; }, base + 2350);
    this._finaleLater(() => { tally.classList.remove('show'); this._finaleReveal(1); }, base + 3650);
    this._finaleLater(() => this._finaleFinish(), base + 4300);
  },

  // 縮放：畫布是固定比例（1280×960＝4:3，跟 finale-background-v3.png 的實際像素一致），
  // 儘可能撐滿視窗、比例不合的一邊留黑邊。跟總決算模擬 v1.2 同一套算法。
  _finaleFitStage() {
    const stage = document.getElementById('stage'); if (!stage) return;
    const s = Math.min(innerWidth / 1280, innerHeight / 960);
    stage.style.setProperty('--fit', s.toFixed(4));
  },

  // 中途被打斷（例如群主延長年數，遊戲從總決算畫面收回去繼續玩）要收乾淨：
  // 停掉還沒觸發的計時器與動畫幀，不然畫面關掉後排名還會在背景繼續「揭曉」。
  cancelFinale() {
    this._finaleTimers.forEach(clearTimeout); this._finaleTimers = [];
    cancelAnimationFrame(this._finaleRAF);
    this._finaleRunning = false;
    removeEventListener('resize', this._finaleFitStageBound);
  },

  showGameOver(ranking) {
    this._finaleRanking = ranking;
    document.getElementById('gameover').style.display = 'flex';
    if (!this._finaleFitStageBound) this._finaleFitStageBound = () => this._finaleFitStage();
    addEventListener('resize', this._finaleFitStageBound);
    this._finaleFitStage();
    document.getElementById('replay').onclick = () => { this._finaleRunning = false; document.getElementById('stage').classList.remove('running'); void document.getElementById('stage').offsetWidth; this._finaleRun(); };
    document.getElementById('skip').onclick = () => {
      if (!this._finaleRunning) return;
      if (!this._finaleFinished) { this._finaleTimers.forEach(clearTimeout); this._finaleTimers = []; this._finaleFinish(); }
    };
    this._finaleRun();
    this.focusOverlayFirst();
  },
