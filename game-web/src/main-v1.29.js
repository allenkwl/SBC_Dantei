...
addEventListener('DOMContentLoaded', () => {
  ...
  // 文件捕捉階段搶最前面處理，理由跟 ui.js 的統一輸入層、上面的虛擬鍵盤一樣：
  ...
  // 玩家設定可全程以方向鍵與 A／空白鍵操作：畫面上每位玩家的大頭貼／改名／模式是同一排橫向排列，
  // 年數、快速模式、開始按鈕各自獨立一排——所以左右／上下不能像以前那樣共用同一條「±1」的
  // 一維清單（那樣按下鍵只會變成跟按右鍵一樣，選到同一位玩家的下一個大頭貼，跳不到下一排）。
  // 改成跟卡片商店、測試模式同一套「依畫面實際位置找最近的那個」幾何導覽：左右在同一排內移動
  // （大頭貼→改名→模式），上下跳到下一排（下一位玩家→年數→快速模式→開始遊戲）。
  //
  // 快速模式勾選框後面接的「萬元」輸入框、年數選單的「自訂年數」輸入框，沒勾選/沒選自訂時
  // 用 display:none 藏起來——舊版用 [style*="display: none"]（冒號後面有空格）比對，但實際
  // style 字串是 display:none（沒有空格），永遠比對不到，導致這兩個隱藏欄位一路都算在清單裡；
  // 按方向鍵移過去時 focus() 對一個 display:none 的元素完全沒反應（瀏覽器不允許 focus 隱藏元素），
  // 焦點就卡住不動，變成「按下下不去、按上也上不來」。改用 offsetParent 判斷是否真的顯示在畫面上，
  // 不比對 CSS 字串，才不會再遇到這種格式不match的問題。
  function rosterControls() {
    return Array.from(document.querySelectorAll('#roster .roster-avatar-opt, #roster .roster-name, #roster .roster-mode, #roster-years-select, #roster-quickwin-toggle, #roster-years-custom, #roster-quickwin-target, #roster-start'))
      .filter(el => el.offsetParent !== null);
  }
  // 表單是「一排一排往下」的結構，不能用卡片商店那種格狀「找畫面上最近的點」演算法——
  // 快速模式勾選框靠左、年數選單置中、開始按鈕置中，橫向位置對不齊，找最近點的演算法
  // 會覺得置中的開始按鈕比靠左的勾選框「更接近正下方」，直接跳過整整一排。
  // 改成先按 Y 座標把控制項分排（同一排＝大頭貼＋改名＋模式），排內再按 X 座標排序：
  // 左右在同一排內移動（到頭就停住，不會跳到別排）；上下一定切到緊鄰的下一排/上一排，
  // 絕對不會因為橫向位置對不齊而跳過某一整排。
  function rosterRows() {
    const items = rosterControls();
    const rows = [];
    items.forEach(el => {
  ...
  function focusRosterGrid(dx, dy) {
    const rows = rosterRows(); if (!rows.length) return;
    ...
    if (dx !== 0) {
      ...
      const next = Math.max(0, Math.min(rows[ri].items.length - 1, ci + dx));
      rows[ri].items[next].focus();
      return;
    }
    const newRi = ri + dy;
    if (newRi < 0 || newRi >= rows.length) return;   // 已經是第一排/最後一排，上下到頭就不動
    const curX = cur.getBoundingClientRect().left;
    let best = rows[newRi].items[0], bestDist = Infinity;
    rows[newRi].items.forEach(el => {
      const d = Math.abs(el.getBoundingClientRect().left - curX);
      if (d < bestDist) { bestDist = d; best = el; }
    });
    best.focus();
  }
  // 文件捕捉階段搶最前面處理，同上；改名欄位以外的按鈕（大頭貼、難度選單…）真的有 focus 時，
  // 真人鍵盤按確定/方向鍵一樣要保證先被這裡接住，不會被瀏覽器原生預設行為搶先。改名欄位本身
  // 不受影響——非確定鍵一律 return 不呼叫 preventDefault，瀏覽器照樣正常打字，跟階段無關。
  document.addEventListener('keydown', e => {
    ...
    if (e.key === 'ArrowLeft')  { e.preventDefault(); focusRosterGrid(-1, 0); return; }
    if (e.key === 'ArrowRight') { e.preventDefault(); focusRosterGrid(1, 0); return; }
    if (e.key === 'ArrowUp')    { e.preventDefault(); focusRosterGrid(0, -1); return; }
    if (e.key === 'ArrowDown')  { e.preventDefault(); focusRosterGrid(0, 1); return; }
    if (!isConfirm) return;
    e.preventDefault();
    ...
    data.players.forEach(p => {
      ...
      div.innerHTML = `
        <div class="sd-player-head">
          ...
          <span class="sd-player-money">💰${formatMoney(p.money)}</span>
        </div>
        <div class="sd-player-stalls">${formatStalls(stationNames)}</div>
      `;
  ...
  document.getElementById('save-slots-back').onclick = () => {
    document.getElementById('save-slots').style.display = 'none';
    // 從人數選擇畫面直接點「讀取存檔」進來的話（saveMode==='load'），退回去要把那個畫面叫回來；
    // 從玩家設定畫面按「開始遊戲」進來選檔案匣的話（saveMode==='assign'），退回去要叫回玩家設定畫面。
    // 這兩種進入方式背景留著的畫面不一樣，只處理其中一種的話，另一種按返回會三個畫面都
