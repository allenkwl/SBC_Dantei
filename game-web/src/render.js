...
const Render = {
  ...
  init(canvas) {
    ...
    // 新版列車圖（新畫風＋合成兩截車廂）已正式取代舊版：
    // local/hsr 直接改指向 v2 圖檔；自強號改用普悠瑪外觀（普悠瑪本來就是自強號等級的傾斜式列車）。
    // 舊圖保留在 *_old 這幾個 key，只給 vehicle_test.html 做新舊比較用，正式遊戲不會用到。
    const TRAIN_FILE = {
      local: 'train_local_v2', local2: 'train_local_v2', local_old: 'train_local_clean',
      hsr: 'train_hsr_v2', hsr2: 'train_hsr_v2', hsr_old: 'train_hsr',
      tzechiang: 'train_puyuma', puyuma: 'train_puyuma', tzechiang_old: 'train_tzechiang',
      // 普通車 Q 版三個候選（AI 產圖去背），只給 vehicle_test.html 比較用，還沒決定要不要正式採用
      q_a: 'train_local_q_a', q_b: 'train_local_q_b', q_c: 'train_local_q_c',
      // 扁平風四車候選（使用者提供的「列車.jpg」切出來的四排，已經是兩截車廂格式），
      // 同樣只給 vehicle_test.html 比較用
      flat_local: 'train_flat_local', flat_hsr: 'train_flat_hsr',
      flat_express: 'train_flat_express', flat_tzechiang: 'train_flat_tzechiang',
      // 使用者提供「列車-黑邊.png」重繪版，本身已經是乾淨的黑色描邊線稿（不是靠程式疊外框），
      // 只給 vehicle_test.html 比較用；getTintedTrain 對這幾個 key 會跳過疊外框那一步，見下方 NO_AUTO_OUTLINE
      black_local: 'train_black_local', black_hsr: 'train_black_hsr',
      black_express: 'train_black_express', black_tzechiang: 'train_black_tzechiang',
    };
    ['local','local2','local_old','express','tzechiang','tzechiang_old','hsr','hsr2','hsr_old','tourism','puyuma',
     'q_a','q_b','q_c','flat_local','flat_hsr','flat_express','flat_tzechiang',
     'black_local','black_hsr','black_express','black_tzechiang'].forEach(k => {
      const img = new Image();
  ...
  // 這個函式只准用 drawImage／fillRect／globalCompositeOperation，絕對不能叫用
  // getImageData／putImageData／toDataURL 這類「讀回像素」的 API：遊戲是直接用 file:// 開啟
  // html（使用者雙擊執行檔就打開，沒有跑 http server），Chrome 對 file:// 讀進來的圖片一樣
  // 套用跨來源畫布保護，只要曾經把這種圖畫上 canvas，這個 canvas 就被標記成「已污染」，
  // 之後對它呼叫任何讀像素的 API 都會直接丟 SecurityError。純粹的 drawImage 疊圖（即使套用
  // 混合模式）屬於「只寫不讀」，瀏覽器允許在已污染的 canvas 上繼續做，不會出錯。
  // 這裡曾經加過一個用 getImageData 掃連通元件清雜點的函式，在這個模擬器（不受 file://
  // 污染限制）測試起來正常，但使用者拿去真的用 file:// 開啟時整個 requestAnimationFrame
  // 迴圈在第一次疊列車圖時就直接拋例外掛掉，畫面永遠停在「地圖畫好、車子完全沒畫出來」的
  // 那一格——因為 frame() 是先畫完玩家棋子才在最後一行重新排下一次 requestAnimationFrame，
  // 疊圖那段丟例外，後面重新註冊 rAF 那行永遠執行不到，整個渲染迴圈就此停格。
  // 雜點清理後來改成離線用 Python 處理，直接覆蓋 assets/trains 底下的靜態圖檔，不再依賴
  // 執行期讀像素。
  //
  // 幫玩家圖示疊一圈黑框：往圓周上 32 個角度各偏移畫出黑色輪廓（依原圖透明度剪出剪影），
  // 再把原圖蓋回最上層——地圖背景顏色深淺不一，車身跟環境色太接近時常常糊成一片，加這圈
  // 黑邊能讓車廂輪廓在任何底色上都看得清楚。
  // 疊圈時先把原圖放大 SS 倍再取整數像素偏移，而不是直接在原尺寸上用小數座標偏移——
  // 試過在原尺寸用小數座標，Canvas 對「只搬移不縮放」的 drawImage(img,dx,dy) 這種兩參數
  // 呼叫大多會直接吃最近的整數像素、不會做次像素平滑，疊出來的剪影還是鋸齒狀（尤其輪子
  // 這種圓形，會變成鋸齒齒輪）。放大之後同樣的整數步進在高解析度下顆粒更細，縮小回原尺寸
  // 時瀏覽器的縮圖平滑會把鋸齒抹開，剪影邊界才會真的呈現平滑圓弧。
  outlineSprite(src, width = 2) {
    const SS = 4;   // 超取樣倍率
    const hiW = src.width * SS, hiH = src.height * SS;
    const hi = document.createElement('canvas');
    hi.width = hiW; hi.height = hiH;
    const hg = hi.getContext('2d');
    const STEPS = 32, hiWidth = width * SS;
    for (let i = 0; i < STEPS; i++) {
      const ang = (i / STEPS) * Math.PI * 2;
      const dx = Math.round(Math.cos(ang) * hiWidth), dy = Math.round(Math.sin(ang) * hiWidth);
      hg.drawImage(src, 0, 0, src.width, src.height, dx, dy, hiW, hiH);
    }
    ...
    og.drawImage(hi, 0, 0, hiW, hiH, 0, 0, out.width, out.height);   // 縮小回原尺寸，順便把鋸齒抹平
    og.drawImage(src, 0, 0);
    return out;
  },

  // 圖檔本身已經是乾淨的黑色描邊線稿（例如使用者提供的「列車-黑邊.png」），不需要（也不該）
  // 再疊一層程式產生的外框，否則邊框會變粗一倍、跟原始線稿的線寬不一致。
  NO_AUTO_OUTLINE: new Set(['black_local', 'black_hsr', 'black_express', 'black_tzechiang']),
  ...
  getTintedTrain(key, color) {
    ...
    g.drawImage(src, 0, 0);
    const outlined = this.NO_AUTO_OUTLINE.has(key) ? c : this.outlineSprite(c);
    this.tintCache[cacheKey] = outlined;
    return outlined;
  },
  ...
  getTintedVehicle(key, color) {
    ...
    g.globalCompositeOperation = 'destination-in';
    g.drawImage(src, 0, 0);
    const outlined = this.outlineSprite(c);
    this.tintCache[cacheKey] = outlined;
    return outlined;
  },
  ...
  drawDestMarker(dst, now) {
    ...
    if (onScreen) {
      ...
      const bob = Math.sin(now / 260) * 1.2 * dpr;
      const poleH = 26 * dpr;
      ctx.save();
      ctx.strokeStyle = '#8a6d3b'; ctx.lineWidth = 1.6 * dpr;
      ctx.beginPath(); ctx.moveTo(s.x, s.y); ctx.lineTo(s.x, s.y - poleH); ctx.stroke();
      ctx.beginPath();
      ctx.moveTo(s.x, s.y - poleH + bob);
      ctx.lineTo(s.x + 20 * dpr, s.y - poleH + 6 * dpr + bob);
      ctx.lineTo(s.x, s.y - poleH + 12 * dpr + bob);
      ctx.closePath();
      ctx.fillStyle = '#FFD54A'; ctx.fill();
      ctx.strokeStyle = '#B8860B'; ctx.lineWidth = 1 * dpr; ctx.stroke();
      ctx.restore();
      return;
    }
