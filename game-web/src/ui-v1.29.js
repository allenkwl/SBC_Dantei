...
const UI = {
  ...
  initArrivalShared() {
    ...
    if (!box) return;
    let shared = null;
    try { shared = JSON.parse(localStorage.getItem(this.ARRIVAL_SHARED_KEY)); } catch (_) {}
  ...
  // 骰子散落用的位置表，比照「擲骰動畫模擬」：depth 由後到前分三層，同一層只左右
  // 錯開，不會互相蓋住上方面點數；每次擲骰重新隨機排列前後關係跟各自的飛入路徑。
  ...
  showDice(finalValues, total, done) {
    ...
    const spots = this.shuffleDice(layout)
      .map(spot => ({...spot, x: Math.max(8, Math.min(92, spot.x + (Math.random() * 6 - 3)))}))
      .sort((a, b) => a.depth - b.depth);
    // t1~t5 這幾個 class 只負責指定飛入路徑（見上面 CSS 說明），跟 face-N 一樣是固定
    // 貼在每顆骰子身上的標記，showFaces() 換點數時要保留、不能整個 className 蓋掉。
    const throwClasses = this.shuffleDice(['t1', 't2', 't3', 't4', 't5']);
    const render = values => {
      const cup = document.createElement('div');
      cup.className = `dice-cup dice-${count}`;
      spots.forEach((spot, i) => {
        const die = document.createElement('span');
        die.className = `die face-${values[i]} ${throwClasses[i % throwClasses.length]}`;
        die.style.left = spot.x + '%';
        die.style.top = this.DICE_DEPTH_Y[spot.depth] + '%';
        die.style.zIndex = String(spot.depth + 1);
        cup.appendChild(die);
      });
      el.replaceChildren(cup);
    };
    const showFaces = values => el.querySelectorAll('.die').forEach((die, index) => {
      die.className = die.className.replace(/face-\d+/, `face-${values[index]}`);
    });
