...
const UI = {
  ...
  init() {
    ...
    // Safari 與可 focus 的畫面元件可能先吃掉 window 的快捷鍵；Y／X 在文件捕捉階段先處理。
    // 這兩鍵只在對應的遊戲狀態生效，並停止往下傳遞，避免同一次按鍵切換兩次。
    document.addEventListener('keydown', e => {
      ...
      if (isX && Game.state === 'awaitRoll') {
        ...
        if (pl && !pl.isAI) { e.preventDefault(); e.stopImmediatePropagation(); this.showCardHand(); }
      }
    }, true);
    ...
    addEventListener('keydown', e => {
      ...
      if (document.getElementById('card-hand').style.display === 'flex' || document.getElementById('card-shop').style.display === 'flex' || document.getElementById('card-targets').style.display === 'flex' || document.getElementById('card-dice').style.display === 'flex') {
        ...
        if (isConfirmKey) { e.preventDefault(); this.lockConfirmKey(e); (document.activeElement || this.focusOverlayFirst()).click(); return; }
        if (isBackKey) {
          e.preventDefault();
      ...
      if (menuOpen) {
        if (e.key === 'ArrowUp' || e.key === 'ArrowLeft') { e.preventDefault(); this.moveOverlayFocus(-1); return; }
        if (e.key === 'ArrowDown' || e.key === 'ArrowRight') { e.preventDefault(); this.moveOverlayFocus(1); return; }
        if (isConfirmKey) { e.preventDefault(); this.lockConfirmKey(e); (document.activeElement || this.focusOverlayFirst()).click(); }
        return;
      }
      ...
      if (Game.reachableMode && e.key in ARROW_ANG) {
        e.preventDefault(); Game.chooseReachableDirection(e.key); return;
      }
