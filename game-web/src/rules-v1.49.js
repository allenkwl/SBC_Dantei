...
const Game = {
  ...
  applyNetState(data) {
    ...
    const turnChanged = (this.cur !== data.cur);
    ...
    // 換人了就把鏡頭移到接手那位的列車上。
    // 原本只有 nextPlayer() 裡有 Render.follow，而那是驅動者專屬——交出 token 之後，
    // 新的持有者自己從來沒跑過 nextPlayer，鏡頭就一直停在上一位的位置。
    if (turnChanged && !Render.freeLook) {
