...
const UI = {
  ...
  init() {
    ...
    muteBtn.onclick = () => { muteBtn.textContent = BGM.toggleMute() ? '🔇' : '🔊'; };
    const ARROW_ANG = {ArrowRight: 0, ArrowDown: Math.PI/2, ArrowLeft: Math.PI, ArrowUp: -Math.PI/2};
    // 全域按鍵：A 鍵功能等同空白鍵（確定/觸發目前 focus 的項目），B 鍵＝跳出目前開著的選單畫面
    addEventListener('keydown', e => {
      ...
      if (document.getElementById('stall-shop').style.display === 'flex') {
        ...
        if (isConfirmKey) { e.preventDefault(); document.activeElement && document.activeElement.click(); return; }
        if (isBackKey)    { e.preventDefault(); document.getElementById('ss-skip').click(); return; }
        return;   // 面板開著時，其他鍵（擲骰、捲動、縮放）先不處理，避免同時觸發
      }

      if (document.getElementById('card-hand').style.display === 'flex' || document.getElementById('card-shop').style.display === 'flex' || document.getElementById('card-targets').style.display === 'flex' || document.getElementById('card-dice').style.display === 'flex') {
        if (isBackKey) {
          e.preventDefault();
          if (document.getElementById('card-shop').style.display === 'flex') Game.leaveCardShop();
          else { this.hideCardHand(); this.hideCardTargets(); this.hideDicePicker(); }
          return;
        }
        return;
      }
      ...
      if (document.getElementById('scout-info').style.display === 'flex') {
        if (isBackKey || isConfirmKey) { e.preventDefault(); this.hideScoutInfo(); }
        return;
      }
      ...
      if (isBackKey) {
        ...
        if (hit) { e.preventDefault(); document.getElementById(hit[1]).click(); return; }
      ...
      // 上面這些選單（設定／存檔／變更年數／確認視窗）開著時，其餘按鍵（擲骰、探路、縮放）先不處理
      const menuOpen = ['settings-menu', 'save-slots', 'save-detail', 'extend-years', 'confirm-modal']
    ...
    if (pl) {
      ...
      div.innerHTML = `${avatarImg}
        <div class="p-cur-info">
          <div class="p-cur-name"><b>${pl.name}</b>${pl.isAI ? '<span class="ai-tag">CPU</span>' : ''}</div>
          <div class="p-cur-money">💰${pl.money} 萬</div>
  ...
  showCardShop(pl, st) {
    ...
    document.getElementById('card-shop-title').textContent = `🃏 卡片商店　${pl.name}：${pl.money} 萬元　${pl.cards.length}/${CARD_HAND_LIMIT}`;
    ...
    Game.cardShopItems(shop).forEach(item => {
      ...
      b.innerHTML = `<span>${c.icon}</span><b>${c.name}　${item.price}萬</b><small>${c.text}</small>`;
      b.onclick = () => Game.buyCard(c.id); list.appendChild(b);
    });
    list.insertAdjacentHTML('beforeend', '<h4>賣出卡片（售價 8 折）</h4>');
    if (!pl.cards.length) list.insertAdjacentHTML('beforeend', '<p class="card-empty">目前沒有可賣出的卡片。</p>');
    pl.cards.forEach((id, index) => {
      const c = CARD_BY_ID[id]; if (!c) return;
      const item = Game.cardShopItems(shop).find(x => x.id === id);
      const sell = Math.floor((item ? item.price : c.price || 0) * .8);
      const b = document.createElement('button'); b.className = `card-item card-${c.type}`;
      b.innerHTML = `<span>${c.icon}</span><b>賣出 ${c.name}　+${sell}萬</b><small>${c.text}</small>`;
  ...
  showCardTargets(card, targets, handIndex) {
    ...
    targets.forEach(({p, i}) => {
      ...
      b.textContent = `${p.name}　💰${p.money}萬${p.shield ? '　🛡️防護中' : ''}`;
  ...
  showDestinationCelebration(st, pl, bonus, done) {
    ...
    document.getElementById('dc-arrival-line').textContent = `${pl.name} 搶先抵達！獲得 ${bonus} 萬元`;
  ...
  showAnnualSettlement(data, done) {
    ...
    data.ranking.forEach((p, i) => {
      ...
      row.innerHTML = `<span class="as-rank">${i === 0 ? '👑' : '#' + (i + 1)}</span><span>${p.name}</span><span class="as-total">${p.total.toLocaleString()} 萬元</span><span class="${change >= 0 ? 'up' : 'down'}">${change >= 0 ? '▲' : '▼'} ${Math.abs(change).toLocaleString()} 萬</span>`;
    ...
    this._annualResultTimer = setTimeout(() => {
      ...
      document.getElementById('as-leader-info').textContent = `${winner.name}以 ${winner.total.toLocaleString()} 萬元 暫居第一名`;
  ...
  // 0.8 秒骰子滾動：1 顆是單顆彈跳、2 顆並排互相錯開、5 顆扇狀排開。
  // finalValues 是真正結算值，total 供顯示骰點合計（含後續卡片／狀態修正）。
  // 骰子畫成白色立體方塊＋黑色點數（1 點比照真實骰子習慣塗紅色），3x3 九宮格用 CSS grid 排點數位置
  showDice(finalValues, total, done) {
    const el = document.getElementById('dice');
    const PIP_POS = [[1,1],[2,1],[3,1],[1,2],[2,2],[3,2],[1,3],[2,3],[3,3]];
    const DIE_PIPS = {1:[4], 2:[2,6], 3:[0,4,8], 4:[0,2,6,8], 5:[0,2,4,6,8], 6:[0,2,3,5,6,8]};
    const face = n => (DIE_PIPS[n] || []).map(i => {
      const [c, r] = PIP_POS[i];
      return `<i style="--gc:${c};--gr:${r}"${n === 1 ? ' class="red"' : ''}></i>`;
    }).join('');
    const render = values => {
      el.innerHTML = `<div class="dice-cup dice-${values.length}">${values.map(n => `<span class="die">${face(n)}</span>`).join('')}</div><small>合計 ${total} 點</small>`;
    };
    ...
    this._diceTicker = setInterval(() => render(finalValues.map(() => 1 + Math.floor(Math.random() * 6))), 85);
    this._diceDone = setTimeout(() => {
      clearInterval(this._diceTicker);
      el.classList.remove('rolling'); el.classList.add('settled'); render(finalValues);
      setTimeout(() => { el.className = ''; done && done(); }, 340);
    }, 800);
  },
  ...
  showBranch(fromId, cands, greenId) {
    ...
    reps.forEach(toId => {
      ...
      btn.onclick = () => Game.chooseBranch(toId);
  ...
  showStallShop(st, pl) {
    document.getElementById('ss-title').textContent = `${st.name}　${pl.name} 的資金：${pl.money} 萬元`;
    ...
    st.stalls.forEach((s, i) => {
      ...
      row.innerHTML = `<input type="checkbox" class="ss-item-check" data-idx="${i}"${(owned || !afford) ? ' disabled' : ''}>
        ${s.name}　${owned ? `（${ownerName}）` : s.price + ' 萬元'}`;
    ...
    if (st.stalls && st.stalls.length) {
      st.stalls.forEach(s => {
        ...
        row.innerHTML = `<b>${s.name}</b>
          <span class="si-meta">${s.price} 萬元・收益 ${s.rate == null ? 100 : s.rate}%</span>
  ...
  showGameOver(ranking) {
    ...
    ranking.forEach((r, i) => {
      ...
      row.innerHTML = `<span class="go-rank-num">${i + 1}</span><img class="go-avatar" src="${r.avatar}" alt=""><b>${r.name}</b><span class="go-total">💰${r.total} 萬元</span>`;
      box.appendChild(row);
    });
