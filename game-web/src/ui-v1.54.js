...
const UI = {
  ...
  applyAnnualPhase(ph) {
    if (ph === 'chart' && this._annualPhase === 'result') this._doAdvanceAnnualPhase();
  },
  ...
  advanceAnnualSettlement(src) {
    ...
    // 連線對戰：「每個人都要各自按過一次」是單機同一台輪流玩才成立的規則。
    // 別台的真人按鍵根本到不了這台，那些位子的確認永遠不會出現，門檻湊不齊——
    // 實測就是卡在第一年結算不動。連線改成跟其他所有面板一致：持 token 的那台推進，
    // 其他人跟著看（階段透過 live 的 overlay 同步過去，見 NET_PANELS）。
    if (Game.netGroup) { if (Game.hasToken()) this._doAdvanceAnnualPhase(); return; }
