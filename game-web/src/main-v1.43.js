...
addEventListener('DOMContentLoaded', () => {
    ...
    go() {
      if (this.mode === 'new') {
        ...
        if (this.netGroup) {
          ...
          Game.loadState(this.data, {netGroup: this.netGroup});
        } else {
  ...
  function openRoom() {
    ...
    document.getElementById('room-save').textContent = mine
      ...
      : '你的裝置上還沒有這個群組的記錄';
    document.getElementById('room-start').textContent = Net.isHost ? '開始遊戲' : '等群主開始⋯';
    Net.watchRoom(info => {
  ...
  document.getElementById('room-start').onclick = () => {
    if (!Net.isHost) { UI.toast('等群主開始遊戲'); return; }
    const mine = OnlineSave.read(Net.groupKey);
    if (mine) {
      showConfirm(
