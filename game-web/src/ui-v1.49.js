...
const UI = {
  ...
  closeNetOverlays() {
    ...
    ['destination-celebration', 'annual-settlement', 'stall-shop'].forEach(id => {
      const el = document.getElementById(id);
  ...
  showDestinationCelebration(st, pl, bonus, done) {
    if (!this._netReadOnly) this.netOverlay = {t: 'celebrate', station: st.id, player: Game.cur, bonus};
  ...
  showAnnualSettlement(data, done) {
    if (!this._netReadOnly) this.netOverlay = {t: 'annual', data};
  ...
  clearDice() {
    // 驅動者收起骰子時，live 封包也要跟著不帶骰子，觀察者才會一起收掉。
    // 以前 _liveDice 只有 nextPlayer() 會清，但驅動者在 land() 就把自己的顯示清掉了——
    // 中間這段 live 還帶著骰子，於是「只有自己清掉、別人畫面上一直留著上一輪的點數」。
    // 統一在這個出口處理，land／quitToSetup／nextPlayer 等所有呼叫點一次涵蓋。
  ...
  _syncStallOverlay(stId) {
    if (!this.netOverlay || this.netOverlay.t !== 'stall') return;
    const listEl = document.getElementById('ss-list');
    this.netOverlay.sel = Array.from(listEl.querySelectorAll('.ss-item-check:checked'))
  ...
  showStallShop(st, pl, readOnly) {
    ...
    if (!readOnly) this.netOverlay = {t: 'stall', station: st.id, player: Game.cur, sel: []};
  ...
  hideStallShop() {
    if (this.netOverlay && this.netOverlay.t === 'stall') this.netOverlay = null;
