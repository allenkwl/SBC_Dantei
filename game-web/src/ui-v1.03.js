...
const UI = {
    ...
    if (pl) {
      ...
      div.innerHTML = `${avatarImg}
        <div class="p-cur-info">
          ...
          <div class="p-cur-money${pl.money < 0 ? ' debt' : ''}">💰${pl.money} 萬</div>
  ...
  showAssetSale(pl, done) {
    ...
    document.getElementById('debt-amount').textContent = `目前現金：${pl.money} 萬元（積欠 ${debt} 萬元）`;
    ...
    pl.stalls.forEach((s, i) => {
      ...
      row.innerHTML = `<input type="checkbox" class="debt-item-check" data-idx="${i}" data-sell="${sellPrice}">
        ${s.name}　${s.price} 萬元<span class="sell-price">賣 ${sellPrice} 萬</span>`;
      listEl.appendChild(row);
    });
    ...
    const updateRemaining = () => {
      ...
      if (stillOwe <= 0) {
        remainingEl.textContent = `已選擇變賣 ${sum} 萬元，足夠還清債務！`;
        remainingEl.classList.add('debt-ok');
      } else {
        remainingEl.textContent = `已選擇變賣 ${sum} 萬元，還差 ${stillOwe} 萬元${allChecked ? '（已經沒有物產可以賣了）' : ''}`;
