...
const Game = {
  ...
  toggleReachableRoutes() {
    ...
    if (this.reachableMode) {
      this.reachableMode = false; this._reachableRoutes = null;
    ...
    UI.update();
    UI.toast(`已圈選 ${routes.size} 個可到站：按方向鍵選方向，或直接點站名`);
  },

  chooseReachableDirection(dirKey) {
    ...
    this._reachableRoutes.forEach((route, id) => {
      const st = Data.stations.get(id);
      const vx = st.x - from.x, vy = st.y - from.y;
      const projection = vx * dx + vy * dy;
      const lateral = Math.abs(vx * dy - vy * dx);
      if (projection <= .01 || lateral > projection * 1.25) return;
      const score = projection + lateral * 2;
      if (score < bestScore) { bestScore = score; best = {id, route}; }
    });
    if (!best) { UI.toast('該方向沒有可到達站點'); return true; }
    this.reachableMode = false;
    this._reachableRoutes = null;
    Render.clearReachableStations();
    UI.hideBranch();
    this._autoRoute = best.route.slice(1);
    this.runAutoRoute();
    return true;
  },
