...
const Input = {
  ...
  // 到站慶祝／年度決算這幾個「按 A 才能繼續」的提示出現時，要讓在場所有參賽手把都震一下
  // （不是只有輪到的那個人）——這幾個畫面本來就是全桌一起看的演出，提醒大家「有東西要看、
  // 別急著滑走」，不分是誰的回合。沒有手把震動馬達的裝置會安靜失敗，不影響其他人。
  vibrateAll() {
    Seats.byPlayer.forEach(src => this.vibrate(src));
  },
  ...
  poll() {
    ...
    for (let i = 0; i < pads.length; i++) {
      ...
      this.pulse(tag + 'select', b(map.mute),     src, 'm', 'KeyM');
    }
    requestAnimationFrame(() => this.poll());
  }
};
