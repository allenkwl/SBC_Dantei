...
//  新聞報導：各類型站點的年收入達標稱號
// ────────────────────────────────────────────────
// 要新增一種就在這裡加一列——圖、門檻、文案全部在同一個地方，
// 不必再去改 rules／ui／HTML 三個檔案。
//
// 門檻刻意「每一種各自訂」而不是一律一百萬：六種類型「買滿一站」的年收入
// 差到 90 倍（夜市 420萬、寺廟 37050萬），共用同一個數字的話，
// 夜市買一攤就達標、寺廟卻永遠碰不到，完全不成比例。
// 下面的數字大致是「買滿三到五站」的量。
const TYCOON_TITLES = [
  {key:'nightmarket', type:'夜市',     name:'夜市大亨',       goal:1500,  verb:'榮登',
   img:'assets/news/夜市大亨.jpg',       lead:'香味飄了三條街！',   body:'電鐵的夜市大排長龍，'},
  {key:'company',     type:'公司',     name:'經營之神',       goal:3500,  verb:'榮登',
   img:'assets/news/經營之神.jpg',       lead:'企業界震撼消息！',   body:'電鐵的公司獲利飆升，'},
  {key:'farm',        type:'農村',     name:'農業冠軍',       goal:7500,  verb:'榮登',
   img:'assets/news/農業冠軍.jpg',       lead:'今年大豐收！',       body:'電鐵的田裡結實纍纍，'},
  {key:'park',        type:'國家公園', name:'國家公園守護者', goal:68000, verb:'榮登',
   img:'assets/news/國家公園守護者.jpg', lead:'山林傳來好消息！',   body:'電鐵用心守護山林，'},
  {key:'factory',     type:'工廠',     name:'優良工廠',       goal:3500,  verb:'榮獲',
   img:'assets/news/優良工廠.jpg',       lead:'生產線全開！',       body:'電鐵的工廠品質一流，'},
  {key:'temple',      type:'寺廟',     name:'神威顯赫',       goal:74000, verb:'榮獲',
   img:'assets/news/神威顯赫.jpg',       lead:'香火鼎盛！',         body:'電鐵的廟埕人潮不斷，'},
];
...
const Game = {
  ...
  // 這次「頁面載入」的隨機標記，每重新整理一次就換一個。用在所有「靠序號去重」的
  // 同步欄位上（目前是骰子的 _liveDice.seq）。
  // 為什麼需要它：clientId 是存在 localStorage 的，重整後不變；而各種計數器都只活在
  // 記憶體裡，重整就歸零。兩者相加產生的序號因此會在「某台重整回來」之後整組重複，
  // 讓對方的去重邏輯誤判成「這筆我處理過了」。以前是在 loadState() 裡把對方的
  // _seenDiceSeq 清掉來繞過，但那只涵蓋「連線開新局」，同一局進行中單一玩家斷線
  // 回來根本不會經過那裡——所以改成從源頭保證序號永不重複，就不必在每條流程各補一次。
  _sessionTag: Math.random().toString(36).slice(2, 8),
  ...
  isNetDriver() {
    ...
    // 真的沒人認領（電腦角色）才輪到群主代跑。
    const _c = this.netClaims && this.netClaims[this.cur];
    if (_c && !_c.away) return false;   // away＝主人不在、由群主代跑，不算「別人的角色」
  ...
  // 座位歸屬（誰是真人、誰是電腦）在回合交界定案。
  // 中途變動的來源只有一個：有人掉線被換成電腦、或掉線的人回來重新認領。
  // 兩種都不該在回合進行中生效（見 applyNetState 裡的說明），所以統一收斂到這裡，
  // driver 與 listener 走同一份，不會出現「一邊認為是電腦、另一邊認為是真人」。
  settleSeatOwners(announce) {
    if (!this.netGroup) return;
    const back = [];
    this.players.forEach((p, i) => {
      const c = this.netClaims && this.netClaims[i];
      const isAI = !c || !!c.away;   // 沒人認領、或主人掉線被電腦接手（away）
      if (p.isAI && !isAI) back.push(p.name);   // 電腦手上收回去 → 有人接回自己的角色
      p.isAI = isAI;
      if (isAI && !p.aiLevel) p.aiLevel = 1;
    });
    // 路 A：電腦已經代打過（座位被標 away）、現在主人回來了。
    // 每一台都會各自偵測到這個轉變（netClaims 是全場同步的），所以各自顯示就好，
    // 不要廣播——廣播會讓已經自己顯示過的裝置再跳第二次。
    if (announce && back.length) UI.announceRejoin(back, {takeover: true});
  },

  applyTurnPresentation(prev, opts) {
    if (!prev) return;
    const curChanged = prev.cur !== this.cur;
    // 回合換人＝座位歸屬定案的時機。接回角色的宣布也在這一刻，
    // 「生效」與「宣布」是同一個時間點，不會有「他已經在玩了但還沒宣布」。
    if (curChanged) this.settleSeatOwners(true);
    const monthChanged = (prev.month !== this.month || prev.year !== this.year);

    // 換人：收起上一位的骰子，鏡頭移到接手那位的列車上
    if (curChanged) {
      // 「該出發了」的提示音要在**交通工具真的出現在畫面上**的那一刻響，
      // 而且要是玩家現在搭的那一種（海上→汽笛、空中→廣播三連音、其餘→火車噗噗）。
      //
      // 跨月時中間隔著月曆橫幅（跨年還會先夾一段年度結算，要等按 A、時間不固定），
      // 交給 UI.showMonthBanner 在動畫結束時發聲。這裡除了看 monthChanged，還要看
      // bannerPending：driver 的 nextPlayer() 是「先換人跑演出、才 advanceMonth()」，
      // 跑到這裡月份根本還沒變，光看 monthChanged 會誤判成一般換人而先響一聲，
      // 於是月曆橫幅前後各響一次——就是實際聽到的「月份畫面多播一次」。
      const bannerPending = !!(opts && opts.bannerPending);
      if (!monthChanged && !bannerPending) this.vehicleCue((this.curPlayer() || {}).vehicleMode);
      // 換人＝新的一輪，行進曲收掉、回到季節曲。這一行 driver 與 listener 都會跑到，
      // 所以兩邊在同一個時間點對齊（月曆橫幅緊接在後面，橫幅播放中就是季節曲）。
      this._rolledThisTurn = false;
      this.syncBgm();
      // 「這一回合出過牌了沒」也要在這裡歸零，不能只靠 beginTurn()。
      //
      // cardUsedThisTurn 從來沒有進過同步封包（/live 與權威狀態都沒帶），是純本機的值；
      // 而唯一的重設點 beginTurn() 開頭就 return 掉 listener，於是：
      //   玩家出了一張牌 → 自己這台把旗標設成 true
      //   → 他回合結束，自己這台跑 nextPlayer／beginTurn，清掉的是「下一位」的旗標
      //   → 下次輪到他時，是別台在跑 beginTurn，清掉的是那台上面的副本
      //   → 他自己這台的 true 永遠沒人清 → 從此再也不能出牌，但擲骰照常
      //      （roll() 只看 state），正是使用者回報的「組員不能用卡片卻可以擲骰」。
      // 群主之所以沒事，是因為它那台的旗標剛好都在別人回合被清掉。
      //
      // 全部玩家一起清：這個旗標只有「輪到的那個人」會被讀（useCard 與電腦選牌都只看
      // curPlayer），清別人的沒有副作用，而且任何一台漏掉某次交界也會自己回正。
      this.players.forEach(p => { p.cardUsedThisTurn = false; });
      UI.clearDice();
      if (!Render.freeLook) {
  ...
  finalRanking() {
    return this.players.map(p => {
      ...
      // stalls：物產「處數」，給總決算畫面的明細用（跟 stallsValue 這個「金額」是兩件事，
      // 以前這裡沒有回傳處數，是因為原本的簡易結算畫面用不到）。
      return {name: p.name, avatar: p.avatar, money: p.money, stallsValue,
              stalls: (p.stalls || []).length, total: this.totalAssetsOf(p)};
  ...
  _syncOverlay(ov) {
    ...
    if (sig !== this._shownOverlaySig) {
      ...
      // 面板身分變化就是天然的邊緣觸發點：新聞快報在這裡放一次號角，
      // 不會因為 overlay 每幀重述而重播。
      if (ov && ov.m === 'showNewsReport') UI.newsOpenedByNet();
    }
    // 面板開著期間的變動（勾選、揭曉階段…）交給那張表上各自的 apply，
    // 不再在這裡一個一個 if 判斷面板名稱——加新面板只要在 NET_PANELS 補一列。
    if (ov) UI.applyNetPanelLive(ov);
  },
  ...
  applyNetLive(v) {
    ...
    // HUD 的「骰出：N」讀的是 Game.dice（見 ui 的 roll-info），而觀察者的 roll() 是被
    // 擋掉的、自己不會設這個值——以前只能等下一次完整狀態同步才更新，中間就會停在
    // 上一位玩家（或電腦代打）的點數，跟對方畫面對不起來。這裡跟著 live 一起更新。
    if (v.dice) this.dice = v.dice.total;
    ...
    if (v.dice && v.dice.seq !== this._seenDiceSeq) {
      ...
      // 觀察者這邊「有人擲骰了」的時機就是這裡（骰子序號變了），跟 driver 的 roll()
      // 對稱。清除一樣交給 applyTurnPresentation 的換人分支，兩邊完全同步。
      //
      // 刻意用「序號變了」這個上升緣，而不是看 v.dice 還在不在：後者在物產商店、
      // 抽卡這些面板期間可能已經被收掉，BGM 會在回合中途就跳回季節曲。
      this._rolledThisTurn = true;
    }
  ...
  applyNetState(data) {
    ...
    data.players.forEach((pd, i) => {
      ...
      // 連線時用全域的定義：沒有「任何人」認領這個座位，才是電腦。
      // 但**不在這裡重算**——重算的時機收斂到回合交界（見 settleSeatOwners）。
      // 理由：掉線的人回來重新認領時，如果 isAI 在電腦回合進行到一半就翻成 false，
      // 會變成電腦停手（maybeAutoRoll 看到不是電腦）、回來的人也動不了（token 還在
      // 代跑的那台手上），那個回合直接懸在那裡。
      if (!this.netGroup) p.isAI = !!pd.isAI;
    });
    const _prev = this.turnSnapshot();
    this.year = data.year; this.month = data.month; this.cur = data.cur;
    // state／stepsLeft 只信「活的權威封包」，不信存檔。
    //
    // 讀檔的連線局，群主是把那份**存檔**上傳成權威狀態的，而 watchState 是 on('value')
    // ——一訂閱就立刻收到現有值。存檔沒有 ver 也沒有 by，上面兩道防線都跳過，
    // 於是這一行會把 loadState() 剛設好的 awaitRoll 蓋成存檔裡的殘值。
    //
    // 而那個殘值幾乎一定是不能操作的狀態：autoSave() 是在 advanceMonth() 裡呼叫的
    // （每年 3 月底寫一次），那一刻 beginTurn() 還沒跑，state 還停在上一位玩家回合
    // 結束時的值（moving／cardRevealed…）。結果就是讀檔局第一輪誰都不能動，
    // 而開新局沒事——因為那份初始狀態是程式現產的、不帶殘值。
    //
    // 版本號正是「這是不是活的封包」的現成判準（pushNetState 一定會帶），拿來當條件
    // 就不必另外傳一個旗標，也不會有「忘了設」的漏洞。
    if (data.ver != null) {
  ...
  start(n, config, totalYears, quickWinTarget) {
    this.players = (config && config.length ? config : CHARS.slice(0, n).map(c => ({charKey: c.key}))).map(cfg => {
      ...
      return {
        ...
        propertyDiscount: false, agriBonus: false, titles: {},
      };
    });
    ...
    UI.update();
    this._bgmKey = null; this._rolledThisTurn = false;
    this.syncBgm();
  ...
  beginTurn() {
    ...
    const proceed = () => {
      ...
      UI.update();
      // 這裡是全域唯一「下一位真的可以動了」的時間點，不管前面繞過月曆橫幅、
      // 年度決算、臭屁鬼扣錢還是暫停回合幾層非同步，最後都會收斂到這一行。
      // 把權威狀態與 token 交接都掛在這裡，就不必在每條分支各補一次
      // （補的那種寫法漏掉一條就是全場卡死，已經發生過）。
      this.pushNetState();
      this.handOffToken();
      this.maybeAutoRoll();
    };
  ...
  flyShortcut(pl, card) {
    ...
    UI.showShortcutFlight(pl, card, () => {
      ...
      // 直升機是瞬間移動，沒有經過任何一段航線，所以不會走到 stepTo 那條
      // 「依路線名稱決定交通工具」的路上——從空中航線飛回來，棋子會一直是飛機。
      // 落地就是站在陸上的站點，交通工具收回列車，BGM 也跟著回到季節配樂。
      pl.vehicleMode = 'train';
      this.updateVehicleBGM('train');
      UI.update();
      this.land();
    });
  },
  ...
  swapPlayers(a, b) {
    ...
    // 跟直升機一樣是瞬間移動，沒經過航線，兩邊的交通工具都收回列車
    a.vehicleMode = 'train'; b.vehicleMode = 'train';
    this.updateVehicleBGM('train');
  ...
  roll(fromCard = false) {
    ...
    this.state = 'moving';
    // 擲完骰、開始走 → 換成這位玩家車型的行進曲（升級列車卡只影響本回合，
    // 所以曲子的生命週期剛好就是這一段移動）
    this._rolledThisTurn = true;
    this.syncBgm();
    ...
    // 序號一定要帶上 _sessionTag（見它的宣告）。只用「clientId＋計數器」會撞號：
    // clientId 存在 localStorage、重整後不變，而計數器只在記憶體、重整就歸零，
    // 於是某台重新連回來之後的第一次擲骰又是 `同一個id:1`——觀察者手上的
    // _seenDiceSeq 剛好還記著上一輪的同一個字串，就把它當成「這個我看過了」，
    // 骰子動畫直接不播（使用者回報的「斷線回來後第一次擲骰，別人畫面看不到」）。
    this._liveDice = {seq: who + ':' + this._sessionTag + ':' + this._diceSeq,
                      values: diceValues, total: this.dice};
  ...
  // 回合開始、上船、起飛都要出聲，而且要是「玩家現在搭什麼」的那個聲音。
  // 為什麼要統一入口＋去重：這幾個時機在連線時會擠在同一瞬間——listener 的
  // applyNetState（換人演出）與 applyNetLive（BGM 同步）是兩條各自獨立的訂閱，
  // 誰先到不固定。硬去規定「這條要出聲、那條要閉嘴」漏掉一條就是連響兩聲，
  // 所以改成同一個函式負責，1.5 秒內同一種提示音只響一次，多餘的自然被吃掉。
  _cueKind: '', _cueAt: 0, _vehMode: null,
  vehicleCue(mode) {
    const kind = mode === 'ship' ? 'ship' : mode === 'plane' ? 'plane' : 'train';
    const now = Date.now();
    if (kind === this._cueKind && now - this._cueAt < 1500) return;
    this._cueKind = kind; this._cueAt = now;
    this._vehMode = kind;   // 這個交通工具已經宣布過了，移動途中不用再宣布一次
    if (kind === 'ship') SFX.shipHorn();
    else if (kind === 'plane') SFX.planeChime();
    else SFX.trainWhistle();
  },

  // 這個角色的「貓咪本名」。玩家改過名之後 p.name 是玩家名字，本名只剩 charKey 查得到。
  // 名字沒改過（本名＝顯示名）就回傳空字串，呼叫端才不會印出「探險家（探險家）」。
  charNameOf(p) {
    if (!p) return '';
    const c = CHARS.find(x => x.key === p.key);
    return c && c.name !== p.name ? c.name : '';
  },

  // 車型 → 行進間 BGM。升級列車卡只影響當回合（beginTurn 會把 train 重設回 local），
  // 所以這首曲子的生命週期天然就是「這一回合的移動」。
  // 只有「升級列車卡」的三種快車有行進曲；普通車（local）維持季節曲。
  // key 來自卡片資料（cards_data.js）：
  //   express   區間快車／區間快車週遊券
  //   tzechiang 普悠瑪衝刺／普悠瑪週遊券
  //   hsr       高鐵快攻／高鐵週遊券
  // puyuma／hsr2 是 render 圖檔對照表裡的舊別名，順手一起收，免得改圖檔命名時漏掉。
  TRAIN_BGM: {
    express: 'train_local',
    tzechiang: 'train_puyuma', puyuma: 'train_puyuma',
    hsr: 'train_hsr', hsr2: 'train_hsr',
  },

  // 現在「應該」播哪一首。driver 與 listener 共用這一份推導，兩邊才不會各算一套
  // （之前 listener 走 syncBgm、driver 靠移動時觸發，觸發點不同就分岔——使用者回報的
  // 「listener 的 BGM 和 driver 不一樣」）。
  //
  // 判斷「這一回合擲過骰了沒」用的是 state 而不是另設旗標：state 本來就在每一份
  // 權威狀態裡，listener 天然拿得到，不必再多同步一個東西。回合開始是 awaitRoll，
  // 擲下去就不再是，下一位的 beginTurn 又把它設回 awaitRoll——剛好就是使用者要的
  // 「擲骰後切換、換下一位切回季節」。
  //
  // 「擲過骰了沒」用旗標而不是看 state：回合交界那一段（月曆橫幅、年度結算）state 還
  // 停在上一位玩家的殘值，看 state 會在橫幅播放中誤判成「還在跑」而放行進曲——而使用者
  // 要的正是「顯示月份背景圖時全場統一播季節曲」。旗標的清除點掛在
  // applyTurnPresentation 的換人分支，那是 driver 與 listener 共用的回合交界，兩邊同步。
  _rolledThisTurn: false,
  bgmKeyNow() {
    const season = BGM.SEASON_KEY[Data.seasonOf(this.month)];
    // 回合開頭與回合交界（月曆橫幅、年度結算）一律季節曲。
    // 這裡刻意不看 vehicleMode：那個值會留到下一回合（它記的是「上一段走過什麼航線」），
    // 拿它決定回合開頭的曲子，就會出現「月份剛開始卻在放空中航線配樂」。
    if (!this._rolledThisTurn) return season;
    const pl = this.curPlayer();
    // 快車最優先，不管陸海空。搭了高鐵卡卻在飛機航線上走一段就切成空中配樂，
    // 一個回合裡切來切去很亂（使用者回報）——一趟就是一首。
    const express = this.TRAIN_BGM[(pl && pl.train) || 'local'];
    if (express) return express;
    const mode = (pl && pl.vehicleMode) || 'train';
  ...
  updateVehicleBGM(mode) {
    const key = this.bgmKeyNow();
    this._bgmKey = key;
    BGM.play(key);
    // 剛上船／剛起飛的那一刻各響一次。邊緣觸發看的是「交通工具」有沒有變，
    // 不是「曲子」有沒有變——這兩件事不一樣，用錯會多響一聲：
    //   ‧ driver 的 _bgmKey 開局是 null（syncBgm 只有 listener 會跑），
    //     所以第一次移動一定被判成「換了交通工具」，擲完骰子開始走就又噗一聲
    //   ‧ 有人搭過船之後 _bgmKey 變 'sea'，下一個人在陸上走又會被判成變化
    // 而且回到陸地本來就不該再噗一聲：「該出發了」在回合開始就響過了。
    const changed = mode !== this._vehMode;
    this._vehMode = mode;
    if (changed && (mode === 'ship' || mode === 'plane')) this.vehicleCue(mode);
  },

  // 背景音樂完全由「目前狀態」推導，driver 與 listener 共用同一份。
  // 原本換季／上船／上飛機的配樂只在 driver 的移動流程裡切，listener 只收到座標與月份，
  // 音樂就一直停在上一首——driver 那邊已經夏天了，別台還在放春天（使用者回報的 BGM 不同步）。
  //
  // 記住上一次「推導出來的曲子」而不是看 BGM.current：負債面板之類會臨時插播自己的曲子，
  // 用 BGM.current 比對的話下一幀就會被這裡蓋掉，插播永遠播不出來。
  _bgmKey: null,
  // 觀察者每收到一幀 live 就校正一次。用推導而不是「收到事件才切」：封包掉了也會
  // 自己回正，這是整套同步一貫的做法（見 NET_PANELS 的說明）。
  syncBgm() {
    const key = this.bgmKeyNow();
  ...
  confirmStallPurchases(stationId, indices) {
    ...
    UI.update();
    if (this.checkQuickWin()) return;
    // 新達成稱號 → 先播新聞快報，等這個人按 A 才換人。
    // 稱號本身寫進 p.titles，會跟著 serialize 進權威狀態，所以別台與存檔都有；
    // 面板則靠 NET_PANELS 的 overlay 每幀重述（掉封包也不會有人卡在畫面上）。
    const got = bought.length ? this.checkTycoonTitles(this.cur) : null;
    if (bought.length) this.pushNetState();
    if (got) { UI.showNewsReport(got, pl, () => this.nextPlayer()); return; }
  ...
  nextPlayer() {
    ...
    // cur 回到 0 代表下面就要 advanceMonth()＋播月曆橫幅，提示音留給橫幅播完再響
    this.applyTurnPresentation(_prev, {bannerPending: this.cur === 0});
    if (this.cur === 0) {
    ...
    // 先把「新的 cur、新的月份」推出去，listener 收到就能同步播月曆橫幅、移鏡頭。
    // 但 token 不在這裡交——交接改到「下一位真的可以動了」那一刻，見 beginTurn()。
    //
    // 為什麼不能在這裡交：上面兩條分支的 beginTurn() 一條是同步、一條不是。
    // cur === 0 要換月，beginTurn 被塞在 showMonthBanner 的 animationend 回呼裡，
    // 大約兩秒後才跑；而這裡是同步往下走的，於是變成
    //   ① 推出去的權威狀態 state 還停在上一位打完的值，不是 'awaitRoll'
    //   ② token 立刻交給下一台
    //   ③ 兩秒後橫幅播完、這台回頭要跑 beginTurn，但它已經不是持有者了，
    //      開頭那行 isListener() 直接 return
    // 結果 'awaitRoll' 沒有任何一台設得起來，全場卡死——就是實測到的
    // 「四月玩完、五月一開始就沒有人能操作」。年度決算那條分支更久，同樣會中。
    this.pushNetState();
  },
  ...
  handOffToken() {
    ...
    // 交出去之前，強制再送一幀 live。
    //
    // 面板的「關閉」只透過 live 串流傳達（每一幀重述現況），而 pushNetLive 第一行就是
    // 「不是驅動者就不送」——token 一交出去這條串流立刻停。偏偏收尾動作幾乎都是
    // 「關面板」跟「換人」寫在同一個 tick 裡，例如 revealYellowCard 的
    //   UI.hideCardDraw(); this.nextPlayer();
    // 於是「面板已經關了」這件事永遠等不到下一次推送（節流是 200ms，而交接是同步的），
    // listener 手上最後一幀還開著抽卡畫面，就這樣卡在那裡關不掉。
    // 那台之後輪到自己時，畫面被蓋住、按鍵被面板吃掉，等於全場停擺
    // （實測 GOGOGO 這一局就是這樣掛的）。
    //
    // 這裡補一幀「乾淨的收尾畫面」：這時 nextPlayer 已經把面板關掉、骰子清掉、
    // cur 換好，beginTurn 也把 state 設成 awaitRoll 了，正好是 listener 需要的狀態。
    // 放在 handOffToken 是因為這裡是 token 唯一的離開點，不必在每個關面板的地方各補一次。
    this.pushNetLive(true);
    ...
    // away 的座位主人不在，跟電腦角色一樣交給群主代跑；他回來清掉 away 之後，
    // 下一次交接就會自然回到他手上，不需要另外做「把 token 還給他」的流程。
    if (claim && claim.id && !claim.away) { assign(claim.id); return; }   // 真人：交給認領那台
  ...
  advanceMonth() {
    ...
    // 這裡以前是 BGM.playSeason()——直接播、不更新 _bgmKey，於是 driver 停在季節曲、
    // listener 卻照自己的推導播別的（使用者回報「月份開始 driver 是季節曲、
    // listener 是空中航線曲」）。改成走同一份推導，兩邊必然一致。
    this.syncBgm();
    return annualData;
  },
  ...
  // 三月結算的收益公式只有這一份。
  // 新聞報導的達標判斷（見 checkTycoonTitles）也呼叫這裡，刻意不另外複製一份——
  // 複製出去之後，報導說「你達標了」而實際結算卻不是那個數字，遲早會對不上，
  // 那正是這套程式一路修過來最常見的那類 bug。
  //
  // opts.type 只在「算某一種站點類型的收入」時給（新聞報導用）。
  // 注意全縣制霸的判定一定要用玩家的**全部**持有來算，不能只看那個類型，
  // 所以類型過濾只作用在最後累加的那一步。
  stallRevenue(i, opts) {
    const p = this.players[i];
    if (!p || !p.stalls || !p.stalls.length) return {total: 0, dominated: 0};
    const onlyType = opts && opts.type;
    ...
    Object.entries(byStation).forEach(([stationId, owned]) => {
      const st = Data.stations.get(stationId);
      if (onlyType && (!st || st.type !== onlyType)) return;
    ...
    return {total, dominated: dominatedCities.size};
  },

  // 買完物產後檢查有沒有新達成的稱號。回傳第一個新達成的（一次只播一則新聞，
  // 同時達成兩種的話下一次買完再播——連著播兩則反而看不清楚）。
  checkTycoonTitles(i) {
    const p = this.players[i];
    if (!p) return null;
    p.titles = p.titles || {};
    for (const t of TYCOON_TITLES) {
      if (p.titles[t.key]) continue;                       // 已經拿過就不再播
      if (this.stallRevenue(i, {type: t.type}).total < t.goal) continue;
      p.titles[t.key] = true;
      return t;
    }
    return null;
  ...
  settleRevenue() {
    const lines = [];
    this.players.forEach((p, i) => {
      const {total, dominated} = this.stallRevenue(i);
      if (total > 0) {
        p.money += total;
        lines.push(`${p.name} +${formatMoney(total)}${dominated ? '（全縣制霸！）' : ''}`);
      }
    });
  ...
  setTotalYears(newTotal) {
    ...
    if (this.state === 'gameover' && this.year <= this.totalYears) {
      this.state = 'awaitRoll';
      // 總決算畫面自己有計時器、動畫、BGM（見 UI.showGameOver）；直接把面板關掉不會
      // 連帶停掉這些——BGM 會一直卡在慶祝曲、殘留的 setTimeout 到時間還是會繼續
      // 揭曉排名。UI.cancelFinale() 統一收尾，跟開啟時是同一組狀態。
      if (typeof UI.cancelFinale === 'function') UI.cancelFinale();
      document.getElementById('gameover').style.display = 'none';
      this._bgmKey = null;   // 讓 syncBgm() 不會誤判「曲子沒變」而略過，強制切回目前季節配樂
      this.syncBgm();
      UI.update();
      this.maybeAutoRoll();
    } else {
  ...
  serialize() {
    return {
      savedAt: Date.now(),
      // 這一份除了寫進 localStorage，也會直接交給 Firebase 的 set()。
      // 兩者對 undefined 的處理完全相反：JSON.stringify 安靜忽略，Firebase 則是
      // 「整份丟錯、一個字都不寫」，而錯誤在好幾條路上都被 .catch() 吃掉，
      // 結果就是狀態沒上傳、大家載到舊的那份。所以這裡一律給 null，不給 undefined。
      netName: this.netGroupName || null,
      totalYears: this.totalYears || 5,
      ...
      players: this.players.map(p => ({
        ...
        // 交通工具與軌跡以前只在 /live 裡，權威狀態沒帶。而 /live 只有 driver 會推，
        // 所以斷線回來的那台（走 loadState，不是 live）會拿不到：棋子一律變回火車、
        // 軌跡只剩站點那一個點 → render 的 angAtEnd() 找不到第二個點，車頭朝向直接
        // 回到預設的向右（使用者回報「飛機變成火車，連方向都沒存到」）。
        // 而且如果回來就輪到他，他自己就是 driver，沒有人會推 live 給他，錯的狀態
        // 會一直留著。所以這兩個要進權威狀態。
        vehicleMode: p.vehicleMode || 'train',
        tr: (p.trail || []).slice(-8).map(pt => [Math.round(pt.x), Math.round(pt.y)]),
        ...
        titles: p.titles || {},   // 已達成的新聞稱號；不進權威狀態的話別台會重複播同一則
      })),
    };
  },
  ...
  autoSave() {
    // 沒有玩家就不要存。這種空存檔會把整個群組的記錄毀掉而且沒有任何徵兆：
    // players 是空陣列 → 上傳到 Firebase 之後整個 key 直接消失（空陣列不會被儲存）
    // → 下次續玩，每台都載到一份「有年月、沒有玩家」的狀態 → 選角畫面標題正常、
    // 卻連一張貓咪卡都長不出來，看起來就是「選貓失敗」。
  ...
  buildFreshState(count, totalYears, quickWinTarget) {
    ...
    return {
      ...
      players: CHARS.map(c => ({
        ...
        propertyDiscount: false, agriBonus: false, nextGhostLoss: 0, titles: {},
      })),
    };
  },

  // slot：單機的檔案匣編號；連線對戰改傳 {netGroup:'群組key'}，兩者只會擇一。
  // 還原車頭軌跡。車頭朝向是 render 從軌跡最後兩個不同點算的，所以至少要兩個點。
  // 最後補上站點座標，確保車頭精準停在站上（載入一定是停在站點的時機）；
  // 若補的點跟前一個相同，angAtEnd() 會自己跳過零長度線段，不影響朝向。
  _trailOf(pd, st) {
    const tr = Array.isArray(pd.tr)
      ? pd.tr.filter(q => Array.isArray(q) && q.length === 2).map(q => ({x: q[0], y: q[1]}))
      : [];
    return tr.length ? tr.concat([{x: st.x, y: st.y}]) : [{x: st.x, y: st.y}];
  },
  ...
  loadState(data, slot) {
    if (slot && typeof slot === 'object' && slot.netGroup) {
      ...
      // 其餘「跨局殘留就會誤判重複」的去重快取一併歸零：
      //   _shownOverlaySig  上一局的面板簽章還在 → 新局開同一個面板不會重開
      //   _lastLive         live 差異比對的基準 → 第一幀跟上一局結尾相同時不會送出
      //   _seenDiceSeq      單純的新局清乾淨而已，不是任何 bug 的解法——別被這行誤導。
      //     骰子序號撞號（對方重整後又從 1 開始，被當成「這個我看過了」）已經在源頭
      //     解決：序號帶了每次載入都不同的 _sessionTag（見它的宣告）。這行以前確實是
      //     為了繞過那個問題而加的，但它只涵蓋「連線開新局」這條路，同一局進行中單一
      //     玩家斷線重整回來根本不會經過 loadState，於是 bug 又以另一種情境出現一次。
      //     教訓：靠「在每條流程補一次清除」去對付撞號，一定會漏；要從序號本身下手。
      this._shownOverlaySig = null;
      this._seenDiceSeq = null;
      this._lastLive = null;
      this._bgmKey = null;
      this._vehMode = null; this._cueKind = ''; this._cueAt = 0;
      this._rolledThisTurn = false;
    } else {
    ...
    this.players = data.players.map(pd => {
      ...
      return {
        ...
        pos: pd.pos, ax: st.x, ay: st.y, train: pd.train || 'local',
        // 這兩個以前是寫死的（一律 train、軌跡只有站點一個點），見 serialize 的說明。
        // 舊存檔沒有這兩個欄位就退回原本的行為，不會壞。
        vehicleMode: pd.vehicleMode || 'train',
        trail: this._trailOf(pd, st),
        money: pd.money, stalls: pd.stalls || [], cards: pd.cards || [],
        ...
        titles: pd.titles || {},   // 已達成的稱號；不帶回來的話讀檔後同一則新聞會再播一次
      };
    });
    ...
    this._bgmKey = null; this._rolledThisTurn = false;
    this.syncBgm();
    // 開局／讀檔時沒有「回合交界」可以掛，先定案一次（不宣布，這不是誰接回角色）
    this.settleSeatOwners(false);
    UI.update();
    this.maybeAutoRoll();
  },
};
