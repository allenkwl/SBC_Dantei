...
const Game = {
  ...
  // 觀察者補軌跡時，一步跨超過這個距離就當成「瞬移」而不是「走過去」（見 applyNetLive）。
  // 正常一步是一個站間距，地圖上大約幾十像素；抓 250 已經遠超過任何正常的一步。
  TRAIL_JUMP_MAX: 250,
  ...
  pushNetLive(force) {
    ...
    const body = {
      ...
      overlay: (typeof UI !== 'undefined' && UI.netOverlay) || null,
  ...
  _syncOverlay(ov) {
    ...
    const sig = ov ? [ov.t, ov.station || '', ov.player != null ? ov.player : ''].join('|') : null;
    if (sig !== this._shownOverlaySig) {
      this._shownOverlaySig = sig;
      UI._netReadOnly = true;
      UI.closeNetOverlays();
      if (ov) {
        const st = ov.station ? Data.stations.get(ov.station) : null;
        const pl = this.players[ov.player] || this.curPlayer();
        if (ov.t === 'stall' && st && pl) UI.showStallShop(st, pl, true);
        else if (ov.t === 'celebrate' && st && pl) UI.showDestinationCelebration(st, pl, ov.bonus || 0, () => {});
        else if (ov.t === 'annual' && ov.data) UI.showAnnualSettlement(ov.data, () => {});
      }
      UI._netReadOnly = false;
    }
    if (ov && ov.t === 'stall') UI.applyStallSelection(ov.sel || []);
  },
  ...
  applyNetLive(v) {
    ...
    (v.players || []).forEach((sp, i) => {
      ...
      // 座標直接跟著對方的動畫走，這就是「看到列車在走」的來源；
      // 順便把軌跡也補上，尾巴才不會斷掉。
      // 沒在移動就把軌跡收成「只有現在這一點」。觀察者的軌跡是每 200ms 取樣拼出來的，
      // 跟驅動者沿著路線逐格畫的不一樣；靜止時若還留著取樣點，車廂會沿著那條取樣線
      // 散開並轉成斜的（`tailAt` 是沿軌跡往回退幾個車身長擺車廂）——開局第一眼就看得到。
      const moving = (v.state === 'moving' || v.state === 'shortcutFlight');
      if (!moving) {
        // 靜止時不能只留一個點：render 的 angAtEnd() 要有兩個點才算得出車頭方向，
        // 只剩一點會回傳 0（正右方），於是觀察者看到的列車永遠朝右（v1.86 的副作用）。
        // 改成記住最後的行進方向，在車頭後方鋪一段夠長的直線——方向正確，
        // 車廂也排得下，而且不會像原本那樣沿著取樣點散開。
        const tr = p.trail || [];
        const b = tr[tr.length - 1], a = tr[tr.length - 2];
        if (a && b && (Math.abs(a.x - b.x) > 1e-6 || Math.abs(a.y - b.y) > 1e-6)) {
          p._netAng = Math.atan2(b.y - a.y, b.x - a.x);
        }
        const ang = p._netAng || 0;
        const L = 240;   // 夠長，讓所有車廂都排得進來
        p.ax = sp.ax; p.ay = sp.ay;
        p.trail = [{x: sp.ax - Math.cos(ang) * L, y: sp.ay - Math.sin(ang) * L},
                   {x: sp.ax, y: sp.ay}];
        return;
      }
      if (p.ax !== sp.ax || p.ay !== sp.ay) {
        p.trail = p.trail || [];
        const last = p.trail[p.trail.length - 1];
        // 跳太遠就把軌跡重設成只有新位置這一點，不要硬接上去。
        // 觀察者的軌跡是每 200ms 取樣一次拼出來的，跟驅動者那邊沿著路線一格一格畫的
        // 不一樣；遇到瞬移（捷徑卡／直升機）、跨海長距離、或剛載入狀態這種一步跨很遠的
        // 情況，硬把新舊兩點接起來會產生一條橫越地圖的長直線，而車廂是「沿著軌跡往回
        // 退幾個車身長」擺的（見 render.js 的 tailAt），結果就是整列車廂散在海上、
        // 還轉成斜的——使用者截圖看到的就是這個。
        if (last && Math.hypot(sp.ax - last.x, sp.ay - last.y) > this.TRAIL_JUMP_MAX) {
          p.trail = [{x: sp.ax, y: sp.ay}];
        } else {
          p.trail.push({x: sp.ax, y: sp.ay});
          if (p.trail.length > 40) p.trail.splice(0, p.trail.length - 40);
        }
        p.ax = sp.ax; p.ay = sp.ay;
      }
    });
  ...
  nextPlayer() {
    ...
    if (this.cur === 0) {
      ...
      const startNewMonth = () => {
        ...
        // 換月動畫是一次性演出，走指令通道讓全場都看得到——它原本寫在 nextPlayer 裡，
        // 而 nextPlayer 是驅動者專屬，所以只有當下拿著 token 的那一台會播。
        this.pushCmd({type: 'monthBanner', month: this.month});
        UI.showMonthBanner(this.month, () => {
          this.beginTurn();
        });
      };
