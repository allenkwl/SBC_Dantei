...
addEventListener('DOMContentLoaded', () => {
  ...
  // 收到演出指令：listener 照著播，不做任何判斷
  function applyNetCmd(c) {
    if (!c || !c.type) return;
    if (c.type === 'celebrate') {
      const st = Data.stations.get(c.station);
      const pl = Game.players[c.player];
      if (st && pl) UI.showDestinationCelebration(st, pl, c.bonus || 0, () => {});
    } else if (c.type === 'annual') {
      if (c.data) UI.showAnnualSettlement(c.data, () => {});
    } else if (c.type === 'stallShop') {
      // 觀察者也看得到別人正在買什麼。他們的按鍵本來就被鎖住、buyStall 之類也擋著，
      // 所以這裡只是「看」；面板由驅動者買完／略過時送 overlayEnd 收掉。
      const st = Data.stations.get(c.station);
      const pl = Game.players[c.player];
      if (st && pl) UI.showStallShop(st, pl);
    } else if (c.type === 'overlayEnd') {
      UI.closeNetOverlays();
    }
  }
