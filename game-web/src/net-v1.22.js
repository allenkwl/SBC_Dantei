...
const Net = {
  ...
  _annualRef: null, _annualCb: null,
  ...
  init() {
    ...
    try {
      ...
      this.clientId = this._stableClientId();
  ...
  openOrReclaim(displayName, playerName) {
    ...
    return ref.once('value').then(snap => {
      ...
      // 節點被下一局吃到」（/live、/pause、/token、/annual、狀態版本號…），
  ...
  uploadState(data) {
    ...
    if (!data || !data.players || !Object.keys(data.players).length) {
      return Promise.reject(new Error('EMPTYSTATE'));
    }
  ...
  // ── 年度結算：每台各自投自己那一格 ──
  // 形狀跟 /ready、/offers 一樣：每台裝置只寫屬於自己的 key，由一台（持 token 的）
  // 看到齊了才往下走。年度結算在單機是「在場每一位玩家都要各自按過一次」，那是同一台
  // 輪流玩才成立的規則——連線時別台的按鍵到不了驅動者，門檻永遠湊不齊（實測卡死）。
  // 票要分階段存：排名、折線圖各要一輪，共用一個節點的話第二輪會帶著第一輪的票直接跳過。
  voteAnnual(phase, idx) {
    if (!this.groupKey) return Promise.resolve();
    return this.db.ref('annual/' + this.groupKey + '/' + phase + '/' + idx).set(true).catch(() => {});
  },
  watchAnnual(cb) {
    this.unwatchAnnual();
    if (!this.groupKey) return;
    this._annualRef = this.db.ref('annual/' + this.groupKey);
    this._annualCb = s => cb(s.val() || {});
    this._annualRef.on('value', this._annualCb);
  },
  unwatchAnnual() {
    if (this._annualRef && this._annualCb) this._annualRef.off('value', this._annualCb);
    this._annualRef = null; this._annualCb = null;
  },
  // 開新局時整組清掉（同名重開的話，上一局的票會讓新局第一次結算直接跳過）
  clearAnnual(key) {
    const k = key || this.groupKey;
    if (!k) return Promise.resolve();
    return this.db.ref('annual/' + k).remove().catch(() => {});
  },
  // 進新階段時由驅動者清掉上一輪的票
  clearAnnualPhase(phase) {
    if (!this.groupKey) return Promise.resolve();
    return this.db.ref('annual/' + this.groupKey + '/' + phase).remove().catch(() => {});
  },
  ...
  submitOffer(offer) {
    if (!this.groupKey) return Promise.resolve();
    // 報上去的記錄有可能被群主採用成全場的權威狀態，所以壞掉的（沒有玩家）不要報，
    // 報了會害全場載到殘缺狀態。降級成「我沒有記錄」，群主就會用群組現有的那份。
    if (offer && offer.data && (!offer.data.players || !Object.keys(offer.data.players).length)) {
      offer = {name: offer.name, progress: -1};
    }
    return this.db.ref('offers/' + this.groupKey + '/list/' + this.clientId)
      .set(stripUndefined(offer)).catch(() => {});
  },
  readOffers() {
    if (!this.groupKey) return Promise.resolve({});
  ...
  GROUP_NODES: ['groups', 'states', 'live', 'claims', 'ready', 'offers', 'bots',
                'token', 'cmds', 'beat', 'pause', 'nudges', 'pings', 'pongs', 'annual', 'debug'],
