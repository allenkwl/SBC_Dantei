...
const UI = {
  ...
  init() {
    ...
    bindPerfBtn('dc-prompt',    () => this.revealNextDestination());   // 兩段演出共用同一個推進函式
    bindPerfBtn('dc-continue',  () => this.revealNextDestination());
    bindPerfBtn('as-prompt',    () => this.advanceAnnualSettlement());
    bindPerfBtn('as-chart-go',  () => this.advanceAnnualSettlement());
    ...
    document.addEventListener('keydown', e => {
      ...
      if (!modalOpen && ['splash', 'setup', 'pick'].some(id => {
        const el = document.getElementById(id);
      ...
      const menuOpen = ['settings-menu', 'save-slots', 'extend-years', 'confirm-modal', 'gameover']
  ...
  showDice(finalValues, total, done) {
    ...
    this._diceDone = setTimeout(() => {
      ...
      dice.forEach((d, i) => { d.el.className = `die face-${values[i]}`; this.landDie(d.el, d.endScale); });
