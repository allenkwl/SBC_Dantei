...
addEventListener('DOMContentLoaded', () => {
  ...
  function stopNetSync() {
    ...
    if (Net.available) {
      ...
      Net.unwatchClaims(); Net.unwatchPause();
  ...
  function applyPause(info) {
    ...
    document.getElementById('pause-name').textContent = info.name || '玩家';
  ...
  function checkDisconnect(info, presentIds) {
    ...
    // 斷線觸發暫停這件事才需要 Game.netGroup：要等真正進了遊戲、liveClaims 才有意義
    // （watchClaims 是 startNetSync 裡才開始訂閱的，選角畫面期間就算查也是空的）。
    if (!Game.netGroup || Game.netPaused) return;
