...
const UI = {
  ...
  init() {
    ...
    addEventListener('keydown', e => {
      ...
      // 測試模式：焦點在下拉選單上時，方向鍵上／下＝直接叫出瀏覽器原生選單讓玩家選（showPicker），
      // 不是切到別的欄位；左右才是在三個下拉選單／傳送鈕之間移動。焦點在卡片清單（格狀排列）時，
      // 方向鍵沿用跟卡片商店同一套「抓畫面上最近的那個」邏輯。
      if (document.getElementById('test-mode').style.display === 'flex') {
        const active = document.activeElement;
        if (active && active.tagName === 'SELECT') {
          if (e.key === 'ArrowLeft') { e.preventDefault(); this.moveOverlayFocus(-1); return; }
          if (e.key === 'ArrowRight') { e.preventDefault(); this.moveOverlayFocus(1); return; }
          if (e.key === 'ArrowUp' || e.key === 'ArrowDown' || isConfirmKey) {
            e.preventDefault();
            if (isConfirmKey) this.lockConfirmKey(e);
            if (active.showPicker) { try { active.showPicker(); } catch (_) {} }
            return;
          }
